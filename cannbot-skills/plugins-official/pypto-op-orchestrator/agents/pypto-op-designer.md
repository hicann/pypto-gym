...
3. skill `pypto-op-develop`'s `references/pypto-kernel-design-format.md` — Layer A–L kernel design format. Starter templates split across three skills: `pypto-op-develop/templates/impl_template.py.tmpl` (Layer G–K, coder-owned); `pypto-golden-generate/templates/golden-template.py.tmpl` (golden skeleton, mathematician/verifier-owned) and `pypto-op-verify/templates/test_template.py.tmpl` (Layer L, verifier-owned)
