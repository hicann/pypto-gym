...
The lint runner invoked at phase completion rejects the following patterns. Read the templates in skill `pypto-op-develop`'s `templates/impl_template.py.tmpl` for full diff examples — they are reproduced here in compact form so you can scan them before submitting a file.
