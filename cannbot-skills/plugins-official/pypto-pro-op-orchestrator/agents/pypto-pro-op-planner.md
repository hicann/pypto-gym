...
- 运行脚本只允许 `python {脚本路径}`、`pypto-pro-op-plan` 明确给出的 `python -c` 收尾自检，
...
- 每个 class 的 `KB_SELECTION.json` 存在、是合法 JSON，且通过 `pypto-pro-op-kb/CONTRACT.md` 的 planner 契约
- `pypto-pro-op-plan` 的收尾自检输出 `OK`
