...
- **禁止语义作弊**（红线，违反即失败）。**核心计算定义**：算子 SPEC 声明的数学语义所对应的计算——即输出值依赖于输入张量数值大小关系的决策步骤（比较、排序、选择、去重、索引重排等）。host 端绝不能承担核心计算；纯 Python 标量运算（参数校验、形状推导、num_cores 计算）、`torch.empty` 分配输出、张量本就连续时的 `.reshape`/`.view` 纯视图以及单次 kernel 调用不构成作弊。`.contiguous()`、`.to()` 或非连续张量上的 `.permute()`/`.transpose()`/`.movedim()` 会派发真实 device kernel，按 wrapper 边界规则默认必须搬进 kernel；只有 DESIGN.md 在 Stage 3 已裁定且记录证据/代价的例外可保留，Stage 4 不得事后新增。**判据是 profile：`op_times.device_kernels` 里任何 `aclnn*` 都是 wrapper 时间**（依据 `pypto-pro-op-kb/constraints/wrapper-boundary.md`）。除"单 kernel / wrapper 单次调用 / kernel 调用不在循环内"三条机械规则外，以下行为均判定为作弊（以计算实质为准，不以命名/措辞为准）：
...
- **未作弊**（红线）：所有的核心计算逻辑集中在单一 kernel 函数内，host 端不做任何核心计算步骤（host 端只做支撑性操作：纯 Python 标量/形状运算、输出分配、num_cores 等参数计算）。**注意本条只判定「是否作弊」，不是 wrapper 允许做什么的清单**——dtype 转换、permute、contiguous 等张量整形即使不算作弊，也受运行时 `$CANNBOT_CONFIG_ROOT/pypto-pro-op-kb/constraints/wrapper-boundary.md` 限制：默认移入 kernel，只有 DESIGN.md 在 Stage 3 已裁定并记录证据/代价的例外可保留；文件中只允许一个 kernel；**`{op}_wrapper` 只调用一次 kernel**（多次调用 kernel 分担计算视为作弊）；**禁止在循环中调用 kernel**（host 端循环 launch kernel 分担计算视为作弊）。**语义判定**按全局硬性规则「禁止语义作弊」的核心计算定义执行——host 端不得出现值依赖变换、kernel 须输出最终结果、不得规格砍单、不得偷换测试输入。判定标准是计算实质，不是命名
