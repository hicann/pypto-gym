...
- **反馈环境异常**：具体分析其返回的证据——若是**硬件问题**（如某卡 bad state / 不可见 / hang），换卡重新调度子代理尝试（通过 `TILE_FWK_DEVICE_ID` 指定其他可用卡）；若是**软件问题**（如 torch_npu / pypto_pro 未安装或初始化失败、CANN 未配置），停止算子调试并向用户反馈环境诊断结果与修复建议（引导用户参考 CANN、torch_npu、PyPTO/PyPTO-Pro 官方安装文档完成环境部署）。不得要求子代理自行修复环境。若需进一步环境诊断，指示子代理加载 skill `pypto-pro-environment-check` 执行 Step 1 smoke 测试。
