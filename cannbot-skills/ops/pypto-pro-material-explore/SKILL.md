...
description: 探索和评估 PyPTO-Pro 算子开发资料。当已有 SPEC.md，且需要查询 API 可用性、dtype、shape、layout、内存和平台约束，查找官方样例、教程与设计指南，或在设计前形成带来源的可行性与 API 映射结论时使用。输出带来源的资料索引和可行性报告；不要直接做 Stage 3 设计或 Stage 4 实现。
---
...
2. **Vector 证据边界**：按 [Vector 选择规范](../../references/performance-constraints.md#强制-2vector-数值计算用-vf-手写)为每个 Vector 步骤记录默认 VF 映射与目标版本 API 依据。本阶段不比较或实验 VF/tile-op；KB 模板例外由后续 Stage 3 根据 `KB_SELECTION.json` 裁定。
...
3. **API 映射**：将算子计算逻辑分解后的每个原子操作映射到对应 API 调用链：
   - **Vector 数值计算**（elementwise、归约、非线性、排序等）：按顶部「Vector 证据边界」记录默认 VF 映射
   ...
   - **未找到直接对应的 vf API**：优先尝试其他 vf API 组合 + 循环结构并记录依据；若已选 KB 模板未明确要求 tile-op 且 VF 仍不可行，标记 unsupported，不自行切换到 `pl.*`
...
**返回**：API 映射表（含 Vector 默认 VF 映射）、API 约束表（含 vf 指令参数语义/寄存器级行为）、动态维度声明方式、环境常量（值 + 来源路径）、证据路径列表
...
2. 以下章节存在且内容不为空：
   - `## 1. 概述`
   - `## 3. API 文档探索`（须包含 §3.1 API 映射结果、§3.2 Vector 实现依据和 §3.3 API 约束）
...
|------|------|
| 输入无法解析 | 引导用户提供公式或代码 |
| API 不存在 | 检查 vf 组合；命中明确 KB 模板要求时再检查对应 tile-op，否则标记 unsupported 并记录证据 |
| 约束不满足 | 标记 ✗，在风险中给出替代方案 |
| 无匹配样例 | 在「参考实现」章节标注「无匹配」，不阻断流程 |
| Vector 映射缺少依据 | 按顶部「Vector 证据边界」补齐 |
