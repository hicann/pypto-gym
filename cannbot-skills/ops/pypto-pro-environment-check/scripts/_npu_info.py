...
def _validate_kv_format(output: str) -> Tuple[bool, List[str]]:
    ...
    table_markers = sum(1 for line in lines if line.strip().startswith(("|", "+", "=")))
    if table_markers > 2:
        warnings.append(
...
def _parse_chip_name_from_line(line: str, target_npu_id: int) -> Optional[str]:
    ...
    Handles both mapping table layouts:
      - 4-col: NPU ID | Chip ID | Chip Logic ID | Chip Name
      - 5-col: NPU ID | Chip ID | Chip Logic ID | Chip Phy-ID | Chip Name
    Chip Name is always the last column and a single token (e.g. Ascend910).
    """
    parts = line.split()
    if len(parts) < 4:
        return None
