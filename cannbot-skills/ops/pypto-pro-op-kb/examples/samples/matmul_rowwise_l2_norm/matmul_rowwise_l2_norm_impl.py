...
# Kernel 10 fuses a 64-by-64 FP32 matmul with row-wise L2 normalisation.
# = kernel-6 handoff + (z^2 -> row_sum -> rsqrt -> row_expand_mul). rsqrt on the ROW-MAJOR view
# of the reduction (DN view faults on scalar ops — layernorm rule). twin_of matmul_rowwise_l2_norm.
import logging
...
def mm_l2norm(
    ...
    with pl.section_vector():
        ...
        red_rm = pl.make_tile(
            pl.TileType(shape=[1, 32], dtype=pl.DT_FP32, target_memory=pl.MemorySpace.Vec), addr=0x8000, size=512
        )  # row-major view of red (for rsqrt)
        ...
        pl.mul(zsq, mm1_res, mm1_res)  # z^2
        pl.row_sum(red, zsq, tmp)  # ss = sum(z^2) over N -> [32,1]
        pl.rsqrt(red_rm, red_rm)  # 1/sqrt(ss) on the row-major view
        pl.row_expand_mul(tile_out, mm1_res, red)  # z * (1/||z||) row broadcast
...
def test_mm_l2norm_gate_a():
    ...
    z = q @ k
    ref = z / z.norm(dim=-1, keepdim=True)
    d = (out - ref).abs().max().item()
    LOGGER.info(f"GATEA mm_l2norm 64x64 maxdiff={d:.3e}")
