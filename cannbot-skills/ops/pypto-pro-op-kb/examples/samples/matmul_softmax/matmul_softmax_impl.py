...
# STATUS: VALIDATED on Ascend a5 NPU (2026-07-18) — corpus twin 7, Gate A. Attention scores+softmax half.
# VALIDATED-CODE-SHA256: e5bd5f19c0ffd409d1ee39bf5b651a02e8d37d540ee5e66a62b389aa28b92dc2
# Gate A was bit-exact on a 64-by-64 FP32 case. Kernel 7 combines the fused matmul handoff
# with the complete row-wise softmax sequence used by the score half of attention.
import logging
...
def mm_softmax(
    ...
    with pl.section_vector():
        ...
        pl.row_max(red, mm1_res, tmp)  # m = max over N
        pl.row_expand_sub(tile_out, mm1_res, red)  # x - m
        pl.exp(tile_out, tile_out)  # exp(x - m)
        pl.row_sum(red, tile_out, tmp)  # s = sum
        pl.row_expand_div(tile_out, tile_out, red)  # / s
...
def test_mm_softmax_gate_a():
    ...
    ref = torch.softmax(q @ k, dim=-1)
    d = (out - ref).abs().max().item()
    LOGGER.info(f"GATEA mm_softmax 64x64 maxdiff={d:.3e}")
