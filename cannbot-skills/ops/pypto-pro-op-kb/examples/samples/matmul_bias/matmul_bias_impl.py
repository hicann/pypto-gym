...
# -----------------------------------------------------------------------------------------------------------
# Kernel 9 fuses a matmul with a per-column FP32 bias broadcast across all rows.
# It follows the kernel-5 handoff and implements addition through subtraction of a negated bias,
# because the API has no dedicated column-expanding add operation.
# twin_of matmul bias/sugar family.
import logging
...
def mm_bias(
    ...
    with pl.section_vector():
        ...
        pl.load(bias_t, bias, [0, 0])  # [1,64] per-column, same for every row
        ...
        pl.mul(negbias, bias_t, -1.0)  # negbias = -bias
        pl.system.wait_cross_core(pipe=pl.PipeType.V, event_id=0)
        pl.col_expand_sub(tile_out, mm1_res, negbias)  # mm1_res - (-bias) == mm1_res + bias (col broadcast)
...
def test_mm_bias_gate_a():
    ...
    mm_bias(q, k, bias, out)
    torch.npu.synchronize()
    ref = q @ k + bias
    d = (out - ref).abs().max().item()
    LOGGER.info(f"GATEA mm_bias 64x64 maxdiff={d:.3e}")
