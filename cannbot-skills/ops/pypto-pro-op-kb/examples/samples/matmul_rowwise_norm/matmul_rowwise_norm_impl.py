...
# STATUS: VALIDATED on Ascend a5 NPU (2026-07-18) — corpus twin 6, Gate A.
# VALIDATED-CODE-SHA256: bab3b9aa1dfe4511b222cc6820c28406908a47ff138410fa4786a59845139b2d
# Gate A validated the 64-by-64 FP32 result to 1.86e-09 maximum absolute error.
# Kernel 6 combines the proven fused handoff with row-sum and row-expanding division operations.
import logging
...
def mm_norm(
    ...
    with pl.section_vector():
        ...
        red = pl.make_tile(
            pl.TileType(shape=[32, 1], dtype=pl.DT_FP32, target_memory=pl.MemorySpace.Vec, layout=pl.DN),
            addr=0x8000,
            size=512,
        )
        tile_out = pl.make_tile(
            pl.TileType(shape=[32, 64], dtype=pl.DT_FP32, target_memory=pl.MemorySpace.Vec), addr=0xA000, size=8192
        )
        off = sub_index * 32
        pl.system.wait_cross_core(pipe=pl.PipeType.V, event_id=0)
        pl.row_sum(red, mm1_res, tmp)  # red[r] = sum over N of mm1_res[r,:]
        pl.row_expand_div(tile_out, mm1_res, red)  # tile_out = mm1_res / red (broadcast)
...
def test_mm_norm_gate_a():
    ...
    if "Ascend950" not in torch.npu.get_device_name():
        ...
        pytest.skip("not a5")
    torch.manual_seed(0)
    q = torch.rand(64, 64, device="npu:0", dtype=torch.float32)  # positive -> positive row sums
    k = torch.rand(64, 64, device="npu:0", dtype=torch.float32)
    out = torch.zeros(64, 64, device="npu:0", dtype=torch.float32)
    mm_norm(q, k, out)
    torch.npu.synchronize()
    z = q @ k
    ref = z / z.sum(dim=-1, keepdim=True)
    d = (out - ref).abs().max().item()
    LOGGER.info(f"GATEA mm_norm 64x64 maxdiff={d:.3e}")
