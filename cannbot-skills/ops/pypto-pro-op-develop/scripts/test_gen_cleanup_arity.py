...
def main() -> int:
    ...
    for label, arity, delivery, want_blocker in CASES:
        ...
        _LOGGER.info("%s %s", "PASS" if ok else "FAIL", label)
    ...
    _LOGGER.info("%s golden with no return stays silent", "PASS" if ok else "FAIL")
    ...
    _LOGGER.info("%s inconsistent golden returns stay silent", "PASS" if ok else "FAIL")
    ...
    _LOGGER.info("%s unparseable delivery stays silent", "PASS" if ok else "FAIL")
