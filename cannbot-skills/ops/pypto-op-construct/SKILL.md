...
- **If `module_count ≥ 2` (L1 path)**:
  ...
  - Honor the staged file chain (rule 14 in `pypto-orchestration-manual`'s `../pypto-orchestration-manual/references/rules.md`).
...
**Hard rule:** In each iteration, extend the production kernel by at most one new semantic module's real PyPTO logic. Everything downstream remains stubbed or fed from golden boundary tensors (see skill `pypto-orchestration-manual`'s `../pypto-orchestration-manual/references/rules.md` → Module-at-a-time enforcement). This hard rule applies only when `module_count ≥ 2`.

### Before writing PyPTO code — consult skill `pypto-general-debug`'s `../pypto-general-debug/references/DEBUG_GUIDEBOOK.md` §9
...
When writing module code, consult skill `pypto-op-develop` (SKILL.md auto-loads) for the impl coding manual: Layer A–L template (skill `pypto-op-develop`'s `../pypto-op-develop/templates/impl_template.py.tmpl`), the design format (skill `pypto-op-develop`'s `../pypto-op-develop/references/pypto-kernel-design-format.md`), and the framework constraint checklist (skill `pypto-op-develop`'s `../pypto-op-develop/references/execution-constraints.md`). The staged module file chain and module-at-a-time enforcement (this skill, `pypto-op-construct`) sit on top of the impl-coding manual.

### Staged module files (mandatory — do this in code)

Materialize each step as `custom/<op>/modules/<op>_module1_impl.py` → `…_module12.py` → … → `…_module1…N.py` (see skill `pypto-orchestration-manual`'s `../pypto-orchestration-manual/references/rules.md` rule 14). Each file is the artifact for that milestone: golden + PyPTO + runnable compare. Suffix = concatenated module indices (`1`, `12`, `123`, …).
...
When skill `pypto-general-debug`'s `../pypto-general-debug/references/DEBUG_GUIDEBOOK.md` strategies and `diagnose_error` do not resolve the issue, escalate to the following subskills in order:
