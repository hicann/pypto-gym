...
#   2. --compare 模式:  从 GOLDEN_PERF_REPORT.md 读 golden 数据 + msprof 采集 PyPTO 算子，计算加速比
...
标准模式 (默认):
 ...
 对比模式 (--compare):
  --compare              启用对比模式：从 GOLDEN_PERF_REPORT.md 读 golden + msprof 采集 PyPTO 算子
  --output-dir=<dir>     算子目录（包含 test_{op}.py, GOLDEN_PERF_REPORT.md）
