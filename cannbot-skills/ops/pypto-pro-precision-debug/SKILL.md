...
| block_dim 与 get_block_num 不一致 | 部分数据未被处理或核间分配不均 | `block_dim` 超过平台上限时 JIT 会截断到上限并告警，但 `pl.get_block_num()` 返回的是原始传入值而非截断后的值；Kernel 内若用 `pl.get_block_num()` 做跨步分片，实际启动核数与分片假设不一致会导致部分 Tile 漏处理或重复处理 | `tutorials/.../multi_core_partitioning_and_Tiling.md`、`api/.../get_block_num.md` |
