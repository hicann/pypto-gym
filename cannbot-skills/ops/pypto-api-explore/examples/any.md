# any kernel reference

> Note: batch 轴 loop 切分；归约轴（last-dim）整块；bool 经 where 转 FP32 求和后与 0.5 比较实现 any。

```python
@pypto.frontend.jit(runtime_options={"run_mode": pypto.RunMode.NPU})
def any_kernel(a: pypto.Tensor(sl, pypto.DT_BOOL), 
    ...
    for i in pypto.loop(batch, name="batch", unroll_list=[1]):
        ...
        r = pypto.gt(sum_val, pypto.full([1] + inner_out, 0.5, pypto.DT_FP32))
