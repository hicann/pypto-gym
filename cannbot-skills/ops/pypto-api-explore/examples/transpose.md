# transpose kernel reference

> Note: batch 轴 loop 搬运；本算子为 metadata/搬运语义（view 取片 + assemble 拼回），无逐元素计算。

```python
@pypto.frontend.jit(runtime_options={"run_mode": pypto.RunMode.NPU})
def transpose_kernel(in_tensor: pypto.Tensor(sl, pypto_dtype),
