...
全部 API 文档。把 `<类>` 换成下方分区名（operation / tensor / config / datatype / symbolic / controlflow / element / others），`<name>` 换成该区下列的某个条目（已含 `pypto-` 前缀）。**缓存在场**读 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/<类>/<name>.md`；**无则在线** `https://pypto.gitcode.com/_sources/api/tensor_api/<类>/<name>.md.txt`。
例：tensor 区取 `pypto-Tensor-set_cache_policy` → 本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/tensor/pypto-Tensor-set_cache_policy.md` / 在线 `.../_sources/api/tensor_api/tensor/pypto-Tensor-set_cache_policy.md.txt`。
各区随版本增删，最新以缓存目录或 `_sources/api/tensor_api/<类>/index.md.txt` 为准。
...
## config 配置
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/config/<name>.md` / 在线 `_sources/api/tensor_api/config/<name>.md.txt` —— `<name>` 取下列之一：
...
## datatype 数据类型
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/datatype/<name>.md` / 在线 `_sources/api/tensor_api/datatype/<name>.md.txt` —— `<name>` 取下列之一：
...
## symbolic 符号/动态 shape
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/symbolic/<name>.md` / 在线 `_sources/api/tensor_api/symbolic/<name>.md.txt` —— `<name>` 取下列之一：
...
## controlflow 控制流
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/controlflow/<name>.md` / 在线 `_sources/api/tensor_api/controlflow/<name>.md.txt` —— `<name>` 取下列之一：

pypto-cond, pypto-function, pypto-is_loop_begin, pypto-is_loop_end, pypto-loop, pypto-loop_unroll


## element 逐元素
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/element/<name>.md` / 在线 `_sources/api/tensor_api/element/<name>.md.txt` —— `<name>` 取下列之一：

pypto-Element_introduction, pypto-Element_constructor, pypto-Element-dtype, pypto-Element-value


## others 其它/互转
本地 `$PYPTO_DEVKIT_DIR/docs/api/tensor_api/others/<name>.md` / 在线 `_sources/api/tensor_api/others/<name>.md.txt` —— `<name>` 取下列之一：
