...
- **Template:** skill `pypto-op-develop`'s `templates/impl_template.py.tmpl` — **mandatory** skeleton for each `*_module*.py` and the integrated kernel (same layers; trim unused sections only with justification below).
