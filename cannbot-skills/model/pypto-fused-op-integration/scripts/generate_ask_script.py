...
parser.add_argument("--profile-dir", type=str, default=None, help="torch_npu profiler trace 输出目录")
args = parser.parse_args()
...
t0 = time.perf_counter()
if args.profile_dir:
    profile_context = torch_npu.profiler.profile(
        activities=[
            torch_npu.profiler.ProfilerActivity.CPU,
            torch_npu.profiler.ProfilerActivity.NPU,
        ],
        on_trace_ready=torch_npu.profiler.tensorboard_trace_handler(
            args.profile_dir, analyse_flag=True
        ),
        record_shapes=False,
...
logging.info(f"  python3 {output_file} --profile-dir ./prof_pypto")
