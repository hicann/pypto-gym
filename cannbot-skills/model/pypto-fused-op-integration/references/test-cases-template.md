...
**统一格式以 `../../../ops/pypto-op-verify/templates/test_cases_template.json` 为准。**
...
当算子有多个输入参数（如 RMSNorm 的 `hidden_states` + `weight`）时，`input` 字段使用命名子对象替代平铺的 `shape`/`dtype`。完整示例见 `../../../ops/pypto-op-verify/templates/test_cases_template.json` 中的 `multi_input_example`。
