...
"""RMSNorm 精度测试脚本 — Phi-3-mini-4k-instruct"""
...
def run_single_case(case_data):
    ...
    eps = inputs.get("eps", {}).get("value", 1e-5)
    dtype = dtype_map[inputs["hidden_states"]["dtype"]]

    hidden_states = torch.randn(inputs["hidden_states"]["shape"], dtype=dtype).npu()
    weight_raw = torch.randn(inputs["gamma"]["shape"], dtype=dtype).npu()

    output_golden = rms_norm_golden(hidden_states.cpu(), weight_raw.cpu(), eps).npu()
...
def test_rms_norm_phi_3_mini_4k_instruct():
