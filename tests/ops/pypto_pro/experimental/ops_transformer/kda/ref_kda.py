...
class KDAStages(NamedTuple):
    """All intermediate and final outputs of the KDA full pipeline.

    Returned by ``RefKDA.full_pipeline`` so that single-stage tests can
    extract the slice they need without re-running upstream golden code.

    Layouts (all BSND unless noted):
        g_cs:         [B, T, HV, K]           within-chunk cumulative gate sum
        L:            [B, T, HV, C]           strictly lower-triangular L matrix
        A_inv:        [B, T, HV, C]           (I + L)^{-1} per chunk
        u:            [B, T, HV, V]           WY representation u = A_inv @ (beta*v)
        w:            [B, T, HV, K]           WY representation w = A_inv @ (beta*exp(g)*k)
        s_snapshots:  [total_chunks, HV, K, V]  state S entering each chunk
        v_corr:       [B, T, HV, V]           u - w @ S per position
        o:            [B, T, HV, V]           final output
        qf:           [B, T, HV, K]           GQA-expanded + scaled queries
        kf:           [B, T, HV, K]           GQA-expanded keys
    """
...
class RefKDA:
    ...
    def gate_cumsum(
        ...
        """
        B, T, HV, Kd = g.shape
        out = torch.zeros(B, T, HV, Kd, dtype=self.dtype, device=g.device)
        gf = g.to(self.dtype)
    ...
    def kkt_kda(
        ...
        """
        B, T, HV, Kd = k.shape
        L_out = torch.zeros(B, T, HV, chunk_size, dtype=self.dtype, device=k.device)
        kf = k.to(self.dtype)
        for bos, eos in _seq_ranges(T, cu_seqlens):
            for j in range(0, eos - bos, chunk_size):
                s, e = bos + j, min(bos + j + chunk_size, eos)
    ...
    def inversion_kda(self, A: torch.Tensor, cs: int, cu_seqlens=None) -> torch.Tensor:
        ...
        """
        B, T, H, _ = A.shape
        out = torch.zeros(B, T, H, cs, dtype=self.dtype, device=A.device)
        Af = A.to(self.dtype)
        for bos, eos in _seq_ranges(T, cu_seqlens):
            for j in range(0, eos - bos, cs):
                ...
                for h in range(H):
                    ...
                    M = np.linalg.inv((np.identity(v) + Ac.cpu().numpy()).astype(np.double))
                    inv = torch.from_numpy(M).to(Ac.device)
    ...
    def wy_kda(
        ...
        B, T, HV, Kd = k.shape
        Vd = v.shape[-1]
        u_out = torch.zeros(B, T, HV, Vd, dtype=self.dtype, device=k.device)
        w_out = torch.zeros(B, T, HV, Kd, dtype=self.dtype, device=k.device)
        kf, vf = k.to(self.dtype), v.to(self.dtype)
        gf, bf, Af = g_cs.to(self.dtype), beta_sig.to(self.dtype), A_inv.to(self.dtype)
        for bos, eos in _seq_ranges(T, cu_seqlens):
            for j in range(0, eos - bos, chunk_size):
                s, e = bos + j, min(bos + j + chunk_size, eos)
                c_len = e - s
                for h in range(HV):
                    kc = kf[0, s:e, h, :]  # [c_len, K]
                    gc = gf[0, s:e, h, :]  # [c_len, K]
                    vc = vf[0, s:e, h, :]  # [c_len, V]
                    bc = bf[0, s:e, h]  # [c_len]
                    A_invc = Af[0, s:e, h, :c_len]  # [c_len, c_len]
    ...
    def recurrent_kda(
        ...
        B, T, HV, Kd = q.shape
        Vd = u.shape[-1]
        o = torch.zeros(B, T, HV, Vd, dtype=self.dtype, device=q.device)

        for bos, eos in _seq_ranges(T, cu_seqlens):
            nc = (eos - bos + chunk_size - 1) // chunk_size
            for h in range(HV):
                S = torch.zeros(Kd, Vd, dtype=self.dtype, device=q.device)
                for ci in range(nc):
                    s = bos + ci * chunk_size
    ...
    def chunk_h_kda(
        ...
        s_snapshots = torch.zeros(n_chunks, HV, Kd, Vd, dtype=self.dtype, device=k.device)
        v_corr_out = torch.zeros(B, T, HV, Vd, dtype=self.dtype, device=k.device)
        ci_base = 0
        for bos, eos in ranges:
            nc = (eos - bos + chunk_size - 1) // chunk_size
            for h in range(HV):
                S = torch.zeros(Kd, Vd, dtype=self.dtype, device=k.device)
                for ci in range(nc):
                    s = bos + ci * chunk_size
                    e = min(s + chunk_size, eos)
                    gc = g_cs[0, s:e, h, :].to(self.dtype)  # [c_len, K]
    ...
    def full_pipeline(
        ...
        Returns:
            KDAStages NamedTuple containing all 6 stage outputs plus the
            GQA-expanded qf/kf, so single-stage tests can extract slices
            without re-running upstream golden computation.
        """
        B, T, H, Kd = q.shape
        HV = v.shape[2]
        G = HV // H
        ...
        return KDAStages(
            g_cs=g_cs, L=L, A_inv=A_inv, u=u, w=w,
            s_snapshots=s_snapshots, v_corr=v_corr, o=o,
            qf=qf, kf=kf,
        )
