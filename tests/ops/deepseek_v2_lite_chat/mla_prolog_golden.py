...
if __name__ == "__main__":
    ...
    hidden = torch.randn(bsz, seq_len, hidden_size, dtype=torch.float16, device='npu:7')
    kv_a_weight = torch.randn(hidden_size, kv_lora_rank + rope_dim, dtype=torch.float16, device='npu:7')
    kv_b_weight = torch.randn(kv_lora_rank, num_heads * 256, dtype=torch.float16, device='npu:7')
    ln_weight = torch.randn(kv_lora_rank, dtype=torch.float16, device='npu:7')
    eps = 1e-6
    cos = torch.randn(seq_len, rope_dim, dtype=torch.float16, device='npu:7')
    sin = torch.randn(seq_len, rope_dim, dtype=torch.float16, device='npu:7')
    pos_ids = torch.arange(seq_len, dtype=torch.long, device='npu:7').unsqueeze(0)
    ...
    print(f"k_nope shape: {k_nope.shape}, dtype: {k_nope.dtype}")
