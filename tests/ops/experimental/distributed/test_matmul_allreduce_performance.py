...
def _do_allreduce(inputs: DoAllreduceInputs):
    ...
    signal_clear_out = pypto.distributed.shmem_clear_signal(
        inputs.shmem_tensor, pred=[inputs.in_tensor_tile])
    barrier_out = pypto.distributed.shmem_barrier_all(
        inputs.shmem_barrier_signal, [data_clear_out, signal_clear_out])
    pypto.set_cube_tile_shapes([8, 8], [128, 256], [256, 512])
    mm_result = pypto.matmul(inputs.in_tensor_tile, inputs.matmul_weight, pypto.DT_FP32, b_trans=True)
    pypto.set_vec_tile_shapes(inputs.view_row_shape, inputs.hidden_size)
    for dyn_idx in range(inputs.world_size):
        put_out = pypto.distributed.shmem_put(mm_result, [0, 0], inputs.shmem_tensor, dyn_idx,
...
def _add_rmsnorm_and_store_output(inputs: AddRmsnormAndStoreOutputInputs):
    ...
    add_out = pypto.add(inputs.all_reduce_out, residual_tile_fp32)
...
def matmul_allreduce_add_rmsnorm_kernel(
    ...
    for bs_idx in pypto.loop(bs_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="bs_idx"):
        ...
        for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
            ...
            all_reduce_out = _do_allreduce(DoAllreduceInputs(
