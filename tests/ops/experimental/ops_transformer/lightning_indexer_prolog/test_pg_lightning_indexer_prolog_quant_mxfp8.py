...
from experimental.ops_transformer.lightning_indexer_prolog.compare_2_1 import precision_compare_triple
from experimental.ops_transformer.lightning_indexer_prolog.lightning_indexer_prolog_quant_mxfp8_impl import (
...
def gen_inputs(dims, dtype=torch.bfloat16, quant_dtype=torch.float8_e4m3fn):
    ...
    w_idx_qb_scale = (
        torch.empty((q_lora_rank // 64, n * d, 2), dtype=torch.float32).uniform_(0, 1).to(torch.float8_e8m0fnu)
    )
    ...
    return {
        "token_x": x,  # bf16
        "q_norm": q_norm,  # mxf8e4m3
        "q_norm_scale": q_norm_scale,  # fp32
        "w_idx_qb": w_idx_qb,  # mxf8e4m3
        ...
        "idx_k_cache": k_cache,  # fp8e4m3 (block_num, block_size, n_kv, d)
...
def indexer_prolog(inputs_initial: dict, dims: dict, platform: str = "cpu"):
    ...
    if platform == "cpu":
        inputs = {
            ...
            if v.dtype in [torch.bfloat16, torch.float16, torch.float8_e4m3fn, torch.float8_e8m0fnu]
            else v.clone().cpu()
    ...
    idx_k_cache = inputs["idx_k_cache"]  # fp8e4m3
    ...
    if platform == "cpu":
        ...
        w_idx_qb_scale_expand = (
            w_idx_qb_scale.transpose(1, 2).reshape(q_lora_rank // 32, n * d).repeat_interleave(32, dim=0)
        )
...
def lightning_indexer_prolog_quant_mxfp8_meta(
    x,
    q_norm,
    q_norm_scale,
    w_qb,
...
def lightning_indexer_prolog_quant_mxfp8_pypto(
    x,
    q_norm,
    q_norm_scale,
    w_qb,
...
def do_test_lightning_indexer_prolog_quant(case_name, is_acl=False):
    ...
    if is_acl:
        ...
        q_fp8e4m3, q_scale, k_fp8e4m3, k_scale, weights = compile_forward(
            x,
            q_norm,
            q_norm_scale,
            w_qb,
        ...
        q_fp8e4m3, q_scale, k_fp8e4m3, k_scale, weights = lightning_indexer_prolog_quant_mxfp8_pypto(
            x,
            q_norm,
            q_norm_scale,
            w_qb,
