...
def chunk_gated_delta_rule_golden(
    ...
    # Iterative inverse: A = (I - A0)^-1 via column-wise update.
    for i in range(1, chunk_size):
        row = A0[..., i, :i].clone()
        sub = A0[..., :i, :i].clone()
        A0[..., i, :i] = row + (row.unsqueeze(-1) * sub).sum(-2)
    A = A0 + torch.eye(chunk_size, dtype=A0.dtype, device=A0.device)

    v_processed = A @ v_beta
    k_cumdecay = A @ (k_beta * g_cum.exp().unsqueeze(-1))

    if initial_state is None:
        state = torch.zeros(B, N, D_k, D_v, dtype=value.dtype, device=value.device)
    else:
        state = initial_state.to(value)

    attn_out = torch.zeros_like(value)
