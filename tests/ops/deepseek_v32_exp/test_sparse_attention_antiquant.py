...
from deepseek_v32_exp.sparse_attention_antiquant_impl import (
...
def _dequant_slc_kv(slc_nope, slc_kv_up, s2_tile_cur, kv_lora_rank, qk_rope_dim, input_dtype):
    """Dequantize int8 cached KV to floating point for attention compute."""
    ...
    return slc_kv_up[:, :kv_lora_rank]


def _gather_kv_offsets(topk_indices_tmp, s2_tile_cur, block_table, b_idx, block_size):
    """Compute global offsets into kvCache from topk indices and block table."""
...
def _gather_slc_nope(slc_nope, nope_cache_2d, offset, s2_tile_cur):
    """Gather nope cache data using computed offsets."""
    for cur_s2_idx in range(s2_tile_cur):
    ...
    for b_idx in range(b):
        ...
        for s1_idx in range(s1):
            ...
            for s2_idx in range(bn_per_batch):
                ...
                offset = _gather_kv_offsets(topk_indices_tmp, s2_tile_cur, block_table, b_idx, block_size)
                _gather_slc_nope(slc_nope, nope_cache_2d, offset, s2_tile_cur)
                vj = _dequant_slc_kv(slc_nope, slc_kv_up, s2_tile_cur, kv_lora_rank, qk_rope_dim, input_dtype)
...
def _gen_topk_indices(actual_seq, topk, b, s_q):
    """Generate top-k sparse indices for SFA golden."""
    slc_actual_seq = []
    for i in range(b):
...
def _gen_quant_cache(block_num, block_size, n_kv, dtype, qk_rope_dim, kv_lora_rank):
    """Generate quantized KV cache data for SFA golden."""
...
def gen_gather_select_attention_golden_aq(dtype, bn1n2s1, is_kn_quant, actual_seq):
    ...
    topk_indices = _gen_topk_indices(actual_seq, topk, b, s_q)
    topk_indices = topk_indices.reshape(b * s_q, n_kv * topk)

    q_bsnd = gen_uniform_data(shape_q, -1, 1, dtype)
    nope_cache_2d = _gen_quant_cache(block_num, block_size, n_kv, dtype, qk_rope_dim, kv_lora_rank)
...
def _get_tile_config(is_p):
    """Get tile configuration for SFA based on platform and mode."""
    if is_p:
...
def do_test_sparse_attention_func_aq(bn1n2s1, actual_seq, input_params, input_data, atten_out, is_p):
    ...
    tile_config = _get_tile_config(is_p)
