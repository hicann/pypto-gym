...
def run_single_case(case_data, device):
    ...
    gate_bias = torch.randn(inputs["gate_bias"]["shape"], dtype=dtype,
                            device=device) if inputs["gate_bias"]["shape"] else None
    up_weight = torch.randn(inputs["up_weight"]["shape"], dtype=dtype, device=device)
    up_bias = torch.randn(
    inputs["up_bias"]["shape"],
    dtype=dtype,
     device=device) if inputs["up_bias"]["shape"] else None
    down_weight = torch.randn(inputs["down_weight"]["shape"], dtype=dtype, device=device)
    down_bias = torch.randn(inputs["down_bias"]["shape"], dtype=dtype,
                            device=device) if inputs["down_bias"]["shape"] else None
