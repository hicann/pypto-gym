...
def run_single_case(case_data, device):
    ...
    hidden_states = torch.randn(inputs["hidden_states"]["shape"], dtype=dtype, device="cpu")
    gamma = torch.randn(inputs["gamma"]["shape"], dtype=dtype, device="cpu")
    eps = inputs["eps"]["value"]

    output_golden = rms_norm_golden(hidden_states, gamma, eps)
