...
def test_pil_builder_ifexp():
    ...
    with TestParser():
        ...
        def true_test_body_selected():
            _ = Expr.str(0) if Expr.true(1) else Expr.str(2)

        @TestParser.test
        def false_test_orelse_selected():
            _ = Expr.str(0) if Expr.false(1) else Expr.str(2)

        @TestParser.test
        def nested_ifexp_in_body():
            _ = (Expr.str(0) if Expr.true(1) else Expr.str(2)) if Expr.true(3) else Expr.str(4)

        @TestParser.test
        def nested_ifexp_in_orelse():
            _ = Expr.str(0) if Expr.false(1) else (Expr.str(2) if Expr.true(3) else Expr.str(4))

        @TestParser.test
        def nested_ifexp_in_orelse_false():
            _ = Expr.str(0) if Expr.false(1) else (Expr.str(2) if Expr.false(3) else Expr.str(4))
...
def test_pil_builder_bin_op():
    ...
    with TestParser():
        ...
        @TestParser.test
        def add():
            _ = Expr.int(0) + Expr.int(1) * Expr.int(2)
            _ = Expr.int(0) - Expr.int(1) // Expr.int(2)
