...
class LightningIndexerPrologBuildConfig:
    ...
    c1_tile: List[List[int]] = field(default_factory=lambda: [[NUM_16, NUM_16], [NUM_256, NUM_256], [NUM_128, NUM_128]])
    v1_tile: List[int] = field(default_factory=lambda: [NUM_1, NUM_256, NUM_128, NUM_128])
    c2_tile: List[List[int]] = field(default_factory=lambda: [[NUM_16, NUM_16], [NUM_256, NUM_256], [NUM_128, NUM_128]])
...
def rope_3d(x: pypto.Tensor, cos: pypto.Tensor, sin: pypto.Tensor,
            tile_config: LightningIndexerPrologTileConfig) -> pypto.Tensor:
    ...
    x_view = pypto.reshape(cast_x, [x.shape[NUM_0], x.shape[NUM_1], x.shape[NUM_2] // NUM_2, NUM_2],
        valid_shape=[x_valid_shape[NUM_0], x_valid_shape[NUM_1], x_valid_shape[NUM_2] // NUM_2, NUM_2])
...
def rope(x: pypto.Tensor, cos: pypto.Tensor, sin: pypto.Tensor,
         tile_config: LightningIndexerPrologTileConfig) -> pypto.Tensor:
    assert len(x.shape) == SHAPE_DIM_2 and len(cos.shape) == SHAPE_DIM_2 and len(sin.shape) == SHAPE_DIM_2
    seq_size = x.shape[NUM_0]
    d_r = x.shape[NUM_1]
    x_dtype = x.dtype
    pypto.set_vec_tile_shapes(tile_config.rope_2d[NUM_0], tile_config.rope_2d[NUM_1])
    cast_x = pypto.cast(x, pypto.DT_FP32)
    ...
    pypto.set_vec_tile_shapes(tile_config.rope_4d[0], tile_config.rope_4d[1],
                               tile_config.rope_4d[2], tile_config.rope_4d[3])
    x_trans = pypto.transpose(x_view, NUM_2, NUM_3)
...
def _create_prolog_tensors(cfg):
    """Create all input/output pypto tensors for lightning indexer prolog."""
    d_bf16 = pypto.DT_BF16
    d_i32 = pypto.DT_INT32
    tensors = {
        'x': pypto.tensor(dtype=d_bf16, shape=[cfg.b, cfg.s1, cfg.dim], name="x"),
        'qr': pypto.tensor(dtype=d_bf16, shape=[cfg.b, cfg.s1, cfg.q_lora_rank], name="qr"),
        'q_w': pypto.tensor(dtype=d_bf16, shape=[cfg.q_lora_rank, cfg.head_num * cfg.head_dim], name="q_w"),
        'k_w': pypto.tensor(dtype=d_bf16, shape=[cfg.dim, cfg.head_dim], name="k_w"),
        'proj_w': pypto.tensor(dtype=d_bf16, shape=[cfg.dim, cfg.head_num], name="proj_w"),
        'ln_w': pypto.tensor(dtype=d_bf16, shape=[cfg.head_dim], name="ln_w"),
...
def _build_meta_dict(cfg, d_bf16, unroll_list):
    """Build the metadata dict for lightning indexer prolog."""
    return {
        "b": cfg.b, "s1": cfg.s1,
        "head_num": cfg.head_num, "head_dim": cfg.head_dim,
        "rope_head_dim": cfg.rope_head_dim, "q_lora_rank": cfg.q_lora_rank,
        "dim": cfg.dim, "n_kv": cfg.n_kv,
        "cache": {"blockSize": cfg.block_size, "block_num": cfg.block_num},
        "tiles": {
            "c1Tile": cfg.c1_tile, "v1Tile": cfg.v1_tile,
            "c2Tile": cfg.c2_tile, "v2Tile": cfg.v2_tile,
            "rope2D": cfg.rope_2d, "rope3D": cfg.rope_3d_vals, "rope4D": cfg.rope_4d,
        },
        "dims": {
            ...
            "ln_w": [cfg.head_dim], "ln_b": [cfg.head_dim],
            "cos/sin": [cfg.b, cfg.s1, cfg.rope_head_dim],
            "k_cache": [cfg.block_num, cfg.block_size, cfg.n_kv, cfg.head_dim],
            "k_cache_index": [cfg.b, cfg.s1],
            "block_table": [cfg.b, cfg.s2_tile // cfg.block_size],
            "query(out)": [cfg.b * cfg.s1, cfg.head_num, cfg.head_dim],
            "weight(out)": [cfg.b * cfg.s1, cfg.head_num],
            "k_cache_out(out)": [cfg.block_num, cfg.block_size, cfg.n_kv, cfg.head_dim],
        },
        "dtype": str(d_bf16),
        "tile_bs": cfg.tile_bs,
...
def build_lightning_indexer_prolog_args(
    ...
    d_bf16 = pypto.DT_BF16
    tensors = _create_prolog_tensors(cfg)

    tile_cfg = LightningIndexerPrologTileConfig(
        c1_tile=cfg.c1_tile, v1_tile=cfg.v1_tile,
        c2_tile=cfg.c2_tile, v2_tile=cfg.v2_tile,
        rope_2d=cfg.rope_2d, rope_3d=cfg.rope_3d_vals, rope_4d=cfg.rope_4d,
    )

    params = LightningIndexerPrologParams(
        b=cfg.b, s1=cfg.s1, dim=cfg.dim, q_lora_rank=cfg.q_lora_rank,
        head_dim=cfg.head_dim, head_num=cfg.head_num,
        rope_head_dim=cfg.rope_head_dim, block_size=cfg.block_size,
        block_num=cfg.block_num, n_kv=cfg.n_kv, s2=cfg.s2_tile, tile_bs=cfg.tile_bs,
    )

    unroll_list: List[int] = [1, 2, 4, 8, 16, 32]

    args = LightningIndexerPrologArgs(
        x=tensors['x'], qr=tensors['qr'], q_w=tensors['q_w'],
        k_w=tensors['k_w'], proj_w=tensors['proj_w'],
        ln_w=tensors['ln_w'], ln_b=tensors['ln_b'],
        cos=tensors['cos'], sin=tensors['sin'],
        k_cache=tensors['k_cache'], k_cache_index=tensors['k_cache_index'],
        block_table=tensors['block_table'],
        query=tensors['query'], weight=tensors['weight'],
        k_cache_out=tensors['k_cache_out'],
        tile_config=tile_cfg, unroll_list=unroll_list, params=params,
    )

    meta = _build_meta_dict(cfg, d_bf16, unroll_list)
    return args, meta
