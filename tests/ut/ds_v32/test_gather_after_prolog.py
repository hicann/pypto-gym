...
def _build_gather_meta(cfg, cache_rows, max_block_per_batch):
    """Build metadata dict for the gather after prolog test case."""
    return {
        "b": cfg.b, "s1": cfg.s1, "n2": cfg.n2,
        "dims": {
        ...
        "params": {
            "blockSize": cfg.block_size, "topk": cfg.topk,
            "numBlocks": cfg.num_blocks, "s2": cfg.s2,
            "dN": cfg.d_n, "dR": cfg.d_r,
        },
        "dtypes": {"cache": str(pypto.DT_FP16), "index": str(pypto.DT_INT32)},
    }
...
def build_gather_args(cfg: BuildConfig = BuildConfig()):
    ...
    (top_k_indices, k_nope_cache, k_rope_cache, block_table,
     act_seqs, gather_res) = _build_gather_tensors(cfg)

    nsa_params = NSASimpleParamsObj(block_size=cfg.block_size, topk=cfg.topk)
    args = GatherInputs(
        top_k_indices=top_k_indices, k_nope_cache=k_nope_cache,
        k_rope_cache=k_rope_cache, block_table=block_table,
        act_seqs=act_seqs, gather_res=gather_res, nsa_params=nsa_params,
        b=pypto.symbolic_scalar(cfg.b), s1=pypto.symbolic_scalar(cfg.s1),
    )

    meta = _build_gather_meta(cfg, cache_rows, max_block_per_batch)
    return args, meta
