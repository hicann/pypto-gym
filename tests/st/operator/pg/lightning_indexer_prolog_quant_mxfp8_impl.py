...
@dataclass
class PrologMxfp8QuantQueryPathInputs:
    t_tile: int
    q_lora_rank: int
    head_num: int
    head_dim: int
...
def _prolog_mxfp8_quant_query_path(inputs: PrologMxfp8QuantQueryPathInputs):
    ...
    q_norm_scale = pypto.view(
        inputs.q_norm_scale_in, [inputs.t_tile, inputs.q_lora_rank // 64, 2],
        [inputs.t_idx, 0, 0])
    pypto.set_cube_tile_shapes([128, 128], [256, 1024], [128, 128])
    q_scaled_mm = pypto.scaled_mm(q_norm, inputs.w_qb_in, inputs.x_dtype, q_norm_scale, inputs.w_qb_scale_in)

    pypto.set_semantic_label("Query-Rope")
    pypto.set_vec_tile_shapes(8, inputs.head_num * inputs.head_dim)
    q_bf16 = pypto.reshape(q_scaled_mm, [inputs.t_tile, inputs.head_num, inputs.head_dim])
...
@dataclass
class PrologMxfp8QuantKeyPathInputs:
    t_tile: int
    h: int
    head_dim: int
    rope_head_dim: int
...
def _prolog_mxfp8_quant_key_path(inputs: PrologMxfp8QuantKeyPathInputs):
    ...
    pypto.set_vec_tile_shapes(128, inputs.head_dim)
    k_rms_norm = pypto.cast(quant_rms_norm(k_proj, inputs.gamma_k_in, -1, 1e-6), inputs.x_dtype)
...
def _prolog_mxfp8_quant_weights_path(t_tile, head_num, head_dim,
                                       x_dtype, t_idx,
                                       x, w_proj_in,
                                       weights_out):
...
@pypto.frontend.jit(
    pass_options={
        # 0 cast_cos/sin, 1 q_rope, 5 q_nope, 7 q_quant
...
def lightning_indexer_prolog_quant(
    ...
    for t_idx, unroll_length in pypto.loop_unroll(0, t, 1, name="IndexerPrologQuantQuantLoop", idx_name="t_idx",
        ...
        rope_cos, rope_sin = _prolog_mxfp8_quant_query_path(
            PrologMxfp8QuantQueryPathInputs(
        ...
        x = _prolog_mxfp8_quant_key_path(
            PrologMxfp8QuantKeyPathInputs(
        ...
        _prolog_mxfp8_quant_weights_path(
