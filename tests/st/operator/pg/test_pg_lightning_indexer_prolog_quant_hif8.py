...
def _gen_hif8_input_tensors(dims, dtype):
    """Generate input weight and data tensors for hif8 gen_inputs."""
    ...
    hadamard_q = torch.empty((d, d), dtype=dtype).uniform_(-1, 1).npu()
    hadamard_k = torch.empty((d, d), dtype=dtype).uniform_(-1, 1).npu()

    return (x, q_norm, q_norm_scale, w_idx_qb, w_idx_qb_scale, w_idx_k, w_idx_proj, gamma,
            cos, sin, hadamard_q, hadamard_k)


def _gen_hif8_cache(dims):
    """Generate cache tensors for hif8 gen_inputs."""
    b = dims["b"]
    s = dims["t"] // dims["b"]
    ...
    return act_seq, block_table, k_cache, k_scale_cache, k_cache_index


def gen_inputs(dims, dtype=torch.bfloat16):
    (x, q_norm, q_norm_scale, w_idx_qb, w_idx_qb_scale, w_idx_k, w_idx_proj, gamma,
     cos, sin, hadamard_q, hadamard_k) = _gen_hif8_input_tensors(dims, dtype)

    act_seq, block_table, k_cache, k_scale_cache, k_cache_index = _gen_hif8_cache(dims)

    return {
        "token_x": x, "q_norm": q_norm, "q_norm_scale": q_norm_scale,
        "w_idx_qb": w_idx_qb, "w_idx_qb_scale": w_idx_qb_scale,
        "w_idx_k": w_idx_k, "w_idx_proj": w_idx_proj,
        "rms_norm_gamma": gamma,
        "cos_idx_rope": cos, "sin_idx_rope": sin,
        "hadamard_q": hadamard_q, "hadamard_k": hadamard_k,
        "idx_k_cache": k_cache, "idx_k_scale_cache": k_scale_cache,
        "idx_k_cache_index": k_cache_index.npu(),
        "idx_block_table": block_table, "act_seq": act_seq,
    }
...
def _hif8_q_side(inputs, dims):
    """Compute Q-side for hif8 indexer_prolog."""
    b, t, n, d = dims["b"], dims["t"], dims["idx_n_heads"], dims["idx_head_dim"]
    s = t // b
    rope_head_dim = dims["rope_head_dim"]
    q_lora_rank = dims["q_lora_rank"]
    x_dtype = inputs["token_x"].dtype
    cos = inputs["cos_idx_rope"].view(-1, 1, 1, rope_head_dim)
    sin = inputs["sin_idx_rope"].view(-1, 1, 1, rope_head_dim)

    q_proj = torch_npu.npu_quant_matmul(
        inputs["q_norm"].view(t, q_lora_rank), inputs["w_idx_qb"].view(q_lora_rank, n * d),
        inputs["w_idx_qb_scale"].view(n * d), pertoken_scale=inputs["q_norm_scale"].view(t),
        x1_dtype=torch_npu.hifloat8, x2_dtype=torch_npu.hifloat8, output_dtype=x_dtype).view(b, s, n, d)
    ...
    q_hadamard = torch.matmul(q_cat, inputs["hadamard_q"])
    q_hif8, q_scale = torch_npu.npu_dynamic_quant(q_hadamard, dst_type=torch_npu.hifloat8)
    return q_hif8, q_scale


def _hif8_k_side(inputs, dims):
    """Compute K-side for hif8 indexer_prolog."""
    b, s = dims["b"], dims["t"] // dims["b"]
    rope_head_dim = dims["rope_head_dim"]
    d = dims["idx_head_dim"]
    x_dtype = inputs["token_x"].dtype
    ...
    k_proj = torch.matmul(inputs["token_x"], inputs["w_idx_k"])
    k_rms_norm = torch_npu.npu_rms_norm(k_proj, inputs["rms_norm_gamma"], epsilon=1e-6)[0]
    ...
    k_hadamard = torch.matmul(k_cat, inputs["hadamard_k"])
    k_hif8, k_scale = torch_npu.npu_dynamic_quant(k_hadamard, dst_type=torch_npu.hifloat8)

    k_cache = inputs["idx_k_cache"].clone()
    k_scale_cache = inputs["idx_k_scale_cache"].clone()
    scatter_update_pa_bsnd(k_cache, k_hif8.reshape(b, s, 1, d), inputs["idx_k_cache_index"], -2)
    scatter_update_pa_bsnd(k_scale_cache, k_scale.reshape(b, s, 1, 1), inputs["idx_k_cache_index"], -2)

    return k_hif8, k_scale, k_cache, k_scale_cache
...
def indexer_prolog(inputs_initial: dict, dims: dict, precision: str = "same"):
    ...
    q_hif8, q_scale = _hif8_q_side(inputs, dims)
    k_hif8, k_scale, k_cache, k_scale_cache = _hif8_k_side(inputs, dims)

    weights = torch.matmul(inputs["token_x"], inputs["w_idx_proj"])
...
def _do_test_reshape_and_get_refs(dims, inputs_data, golden_data, npu_data):
    """Reshape inputs and reference data for do_test."""
    t = dims["t"]
    ...
    x = inputs_data["token_x"].reshape(t, dims["h"])
    ...
    refs = (q_hif8_golden, q_scale_golden, k_cache_golden, k_scale_cache_golden, weights_golden,
            q_hif8_npu, q_scale_npu, k_cache_npu, k_scale_cache_npu, weights_npu)
    return call_args, refs
...
def do_test_lightning_indexer_prolog_quant(case_name, is_acl=False):
    ...
    call_args, refs = _do_test_reshape_and_get_refs(dims, inputs_data, golden_data, npu_data)
    (q_hif8_golden, q_scale_golden, k_cache_golden, k_scale_cache_golden, weights_golden,
     q_hif8_npu, q_scale_npu, k_cache_npu, k_scale_cache_npu, weights_npu) = refs
    ...
    if is_acl:
        ...
        q_hif8, q_scale, k_hif8, k_scale, weights = compile_forward(*call_args)
    else:
        q_hif8, q_scale, k_hif8, k_scale, weights = lightning_indexer_prolog_quant_hif8_pypto(*call_args)
