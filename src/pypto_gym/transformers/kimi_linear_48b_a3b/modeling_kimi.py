# coding=utf-8
# Adapted from: https://huggingface.co/moonshotai/Kimi-Linear-48B-A3B-Instruct/blob/main/modeling_kimi.py
#
...
try:
    ...
    # transformers >= 5.x relocated OutputRecorder to modeling_utils
...
try:
    ...
    # fla (flash-linear-attention) is CUDA/Triton-only; on Ascend NPU we fall
    # back to the pure-torch drop-ins in kimi_fla_compat (same signatures).
...
def index_put_first_axis(x, indices, first_axis_dim):
    ...
    # Note: indexing is used here; torch.scatter can be marginally faster.
    y[indices] = x
    return y
...
class KimiDynamicCache:
    ...
    def __init__(self, config: KimiLinearConfig):
        ...
        linear_layers = [i for i in range(config.num_hidden_layers) if self.layer_types[i] == "linear_attention"]
    ...
    def reorder_cache(self, beam_idx: torch.LongTensor):
        ...
        for layer_idx, _ in enumerate(self.key_cache):
    ...
    def get_mask_sizes(self, cache_position: torch.Tensor, layer_idx: int) -> tuple[int, int]:
        ...
        """
        kv_offset = 0
        # transformers>=5 passes an int query_length here; older passed a
        # cache_position tensor. Support both.
...
class KimiMLAAttention(nn.Module):
    ...
    def __init__(self, config: KimiLinearConfig, layer_idx: int):
        ...
        try:
            ...
            raise ValueError(
                f"Kimi MLA config is not found or not properly formatted: {e}") from e

        if self.q_lora_rank is not None:
            raise ValueError("Kimi MLA requires q_lora_rank to be None")
        ...
        if not self.use_nope:
            raise ValueError("Kimi MLA requires use_nope=True")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attn_impl == "flash_attention_2" and self.q_head_dim != self.v_head_dim:
            value_states = F.pad(
...
def _run_kda(mode, kda_kwargs):
    """Select the KDA backend for this forward and run it -> (o, recurrent_state).

    Factored out of KimiDeltaAttention.forward so our PyPTO dispatch guard
    (USE_PTO_KDA / USE_PTO_KDA_GRAPH) lives here rather than adding branches to
    the vendored forward. KDA PyPTO covers prefill (chunk) only; decode (T<=64,
    'fused_recurrent') always takes the upstream torch path.
    """
    if mode != 'chunk':
        return _dispatch_kda(fused_recurrent_kda, fused_recurrent_kda, kda_kwargs)
    pto = sys.modules.get("kimi_linear_48b_a3b_pto_kernels")
    # NPU-graph capture path: call the registered PyPTO op DIRECTLY (no
    # _dispatch_kda try/except — a host-raised exception is not capturable).
    # Guarded by USE_PTO_KDA_GRAPH (off by default). The bench only sets this
    # for the captured region where cache_params is None, so initial_state and
    # cu_seqlens are None and kda_chunk_pypto's reduced signature applies.
    if pto is not None and getattr(pto, "USE_PTO_KDA_GRAPH", False):
        return pto.kda_chunk_pypto(
            q=kda_kwargs["q"], k=kda_kwargs["k"], v=kda_kwargs["v"],
            g=kda_kwargs["g"], beta=kda_kwargs["beta"],
            initial_state=kda_kwargs["initial_state"], use_qk_l2norm_in_kernel=True)
    primary_fn = chunk_kda
...
class KimiDeltaAttention(nn.Module):
    def __init__(self, config: KimiLinearConfig, layer_idx: int):
        ...
        # NOTE: keep the upstream name "A_log" — it is the public checkpoint key
        # for this layer's gate-decay parameter. Renaming to snake_case breaks
        # weight loading under sharded device-map loading: accelerate's big-model
        # loader matches checkpoint keys to parameter names directly and bypasses
        # the module load hooks, so a key-remap hook silently leaves the gate-decay
        # weights randomly initialized. The G.NAM.01 finding on this name is waived.
    ...
    def _standard_kda_forward(self, hidden_states, attention_mask, cache_params,
        ...
        # torch chunk / recurrent function.
        #
        # NOTE(multi-NPU coverage): the wrapper raises NotImplementedError (caught
        # below) for any KDA layer whose tensors are NOT on the single NPU PyPTO is
        # bound to. So under a single-process device_map shard, only the bound NPU's
        # KDA layers run on PyPTO (e.g. ~6 of 20 with 4-way sharding) and the rest
        # take this torch fallback. The wrapper emits a one-time RuntimeWarning so
...
class KimiMoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            raise RuntimeError("MoE gating top-k selection is inference-only")
...
class KimiLinearModel(KimiPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # transformers>=5 generate may pass a generic DynamicCache; the Kimi
        # linear-attention path needs its own cache type, so coerce it.
        if use_cache and not isinstance(past_key_values, KimiDynamicCache):
        ...
        if past_key_values is not None:
            if not isinstance(past_key_values, KimiDynamicCache):
                raise TypeError("past_key_values must be a KimiDynamicCache")
