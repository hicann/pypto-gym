...
# Modified for Spatial-SSRL-3B with:
#   - Absolute imports (5.6.0 compatible)
#   - PTO RMSNorm injection (sys.modules.get("spatial_ssrl_3b_pto_kernels"))
#   - PTO RoPE injection (apply_rotary_pos_emb_vision_wrapper, apply_multimodal_rotary_pos_emb_wrapper)
# -----------------------------------------------------------------------------------------------------------
...
class Qwen2_5_VLRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # PyPTO RMS Norm injection - spatial_ssrl_3b_pto_kernels
        pto_kernels = sys.modules.get("spatial_ssrl_3b_pto_kernels")
        if pto_kernels is not None and getattr(pto_kernels, "USE_PTO_RMS_NORM", False):
            return pto_kernels.rms_norm_pto_wrapper(hidden_states, self.weight, self.variance_epsilon)
...
def apply_rotary_pos_emb_vision(
    ...
    # PyPTO RoPE injection for vision
    pto_kernels = sys.modules.get("spatial_ssrl_3b_pto_kernels")
    if pto_kernels is not None and getattr(pto_kernels, "USE_PTO_ROPE", False):
        return pto_kernels.apply_rotary_pos_emb_vision_wrapper(q, k, cos, sin)
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    # PyPTO RoPE injection for multimodal
    pto_kernels = sys.modules.get("spatial_ssrl_3b_pto_kernels")
    if pto_kernels is not None and getattr(pto_kernels, "USE_PTO_ROPE", False):
        return pto_kernels.apply_multimodal_rotary_pos_emb_wrapper(q, k, cos, sin, mrope_section, unsqueeze_dim)
