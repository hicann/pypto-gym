# coding=utf-8
# Adapted from
# https://huggingface.co/FreedomIntelligence/openPangu-Embedded-7B/blob/main/modeling_openpangu_dense.py
...
from executor.utils import get_default_group, init_comm_group
...
from pypto_gym.ops.pypto_tensor import openpangu_v5_7b as _pto_ops
...
def check(name, t):
    if torch.isnan(t).any() or torch.isinf(t).any():
        logger.error(
            "[NAN] %s: nan=%s inf=%s min=%s max=%s dtype=%s shape=%s",
            name,
            torch.isnan(t).any().item(),
            torch.isinf(t).any().item(),
            t.nan_to_num().min().item(),
            t.nan_to_num().max().item(),
            t.dtype,
            tuple(t.shape),
        )
    ...
    def forward(self, hidden_states, *args):
        if len(args) == 0: # only hidden_states exists
            ...
            raise NotImplementedError(
                f"insupportable PanguEmbeddedRMSNorm for input_args len as (include hid): {len(args) + 1}"
            )


ALL_LAYERNORM_LAYERS.append(PanguEmbeddedRMSNorm)
...
# Copied from transformers.models.llama.modeling_llama.LlamaLinearScalingRotaryEmbedding with Llama->PanguEmbedded


class PanguEmbeddedLinearScalingRotaryEmbedding(PanguEmbeddedRotaryEmbedding):
    """PanguEmbeddedRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
# Copied from transformers.models.llama.modeling_llama.LlamaDynamicNTKScalingRotaryEmbedding with Llama->PanguEmbedded


class PanguEmbeddedDynamicNTKScalingRotaryEmbedding(PanguEmbeddedRotaryEmbedding):
    """
    PanguEmbeddedRotaryEmbedding extended with Dynamic NTK scaling.
...
class PanguEmbeddedYarnRotaryEmbedding(PanguEmbeddedRotaryEmbedding):
...
class PanguEmbeddedDenseMLP(nn.Module):
    def __init__(self, config, runner_settings, prefix: str = "", **kwargs):
        super().__init__()
        self.config = config
        self.hidden_dim = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.dense_tp_size = runner_settings.get("parallel_config").get("dense_tp_size", 1)
        self.moe_tp_size = runner_settings.get("parallel_config").get("moe_tp_size", 1)
    ...
    def forward(self, hidden_states, is_prefill=False, **kwargs):
        if self.dense_tp_size > 1:
            ...
            hidden_states_output = torch.empty(
                [bsz * self.dense_tp_size, q_len, self.hidden_dim],
            ...
            dist.all_gather_into_tensor(
                ...
                hidden_states,
                group=self.hccl_comm_dict.get("dense_tp_group", None),
            )
            hidden_states = hidden_states_output
        gate_states = self.gate_proj(hidden_states)
        up_states = self.up_proj(hidden_states)
        merged_hidden_states = torch.cat([gate_states, up_states], dim=-1)
        intermediate_hidden_states = torch_npu.npu_swiglu(merged_hidden_states)
        output = self.down_proj(intermediate_hidden_states)
        if self.dense_tp_size > 1:
            mlp_res = output.new_empty(bsz, q_len, output.shape[-1])
            ...
            dist.all_reduce(output, group=self.hccl_comm_dict.get("moe_tp_group", None))
    ...
    def dispatch_double_routing(self, tokens_per_expert, expanded_x, pertoken_scale):
        if self.hccl_comm_dict is not None:
            ...
            if self.gmm_quant_mode == "w8a8":
                ...
                gathered_pertoken_scale = pertoken_scale
        else:
            tokens_per_expert = tokens_per_expert.to(torch.int64)
            tokens_per_expert_group = tokens_per_expert
            gathered_tokens = expanded_x
            combine_tokens = torch.stack([tokens_per_expert_group, tokens_per_expert], dim=0)
            combine_tokens = combine_tokens.view(2, self.moe_ep_size, -1).sum(2)
            gathered_pertoken_scale = pertoken_scale

        return tokens_per_expert_group, gathered_tokens, None, None, gathered_pertoken_scale

    def moe_infer_double_routing(self, hidden_states, topk_ids, topk_weight):
        batch_size, sequence_length, h = hidden_states.shape
        hidden_states = hidden_states.view(-1, h)
        bs_qlen = hidden_states.shape[0]
        is_e4m3 = os.getenv("ENABLE_MXFP", None) is not None
        expanded_x, expanded_row_idx, tokens_per_expert, pertoken_scale = torch_npu.npu_moe_init_routing_v2(
            hidden_states,
            expert_idx=topk_ids,
            active_num=topk_ids.shape[0] * topk_ids.shape[1],
            # scale=self.experts.smooth_scale_1 if self.gmm_quant_mode == "w8a8"
            # and self.quant_type == QuantType.INT8 else None,1
            scale=self.experts.smooth_scale_1 if self.gmm_quant_mode == "w8a8" and not is_e4m3 else None,
            expert_num=self.num_experts,
            expert_tokens_num_type=1,  # 0: cumsum mode(not supported now); 1: count mode
            expert_tokens_num_flag=True, active_expert_range=[0, self.num_experts],
            quant_mode=1 if self.gmm_quant_mode == "w8a8" and not is_e4m3 else -1,
    ...
    def moe_infer_dispatch_combine(self, x, topk_ids, topk_weight):
        ...
        if self.quant_type == QuantType.HIF8:
            expand_x, dynamic_scale = torch_npu.npu_dynamic_quant(expand_x, dst_type=torch_npu.hifloat8)
        ...
        is_e4m3 = os.getenv("ENABLE_MXFP", None) is not None
        is_dynamo = os.getenv("DYNAMO", None) is not None
        if self.gmm_quant_mode == "w8a8" and is_dynamo is None:
            if is_e4m3 or self.quant_type == QuantType.MXFP8:
                dim0, dim1 = dynamic_scale.shape
                dynamic_scale = dynamic_scale.view(dim0, dim1 // 2, 2)
...
def _init_rope(self):
    if self.config.rope_scaling is None or self.config.rope_scaling["rope_type"] == "default":
        self.rotary_emb = PanguEmbeddedRotaryEmbedding(
            self.config,
        ...
        if scaling_type == "linear":
            self.rotary_emb = PanguEmbeddedLinearScalingRotaryEmbedding(
                self.config,
                self.config.head_dim,
                max_position_embeddings=self.max_position_embeddings,
            ...
            self.rotary_emb = PanguEmbeddedDynamicNTKScalingRotaryEmbedding(
                self.config,
                self.config.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=scaling_factor,
            ...
            self.rotary_emb = PanguEmbeddedYarnRotaryEmbedding(
                self.config,
                self.config.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=scaling_factor,
...
class PanguEmbeddedAttention(nn.Module):
    ...
    def __init__(self, config, runner_settings, layer_idx: Optional[int] = None, prefix: str = "", **kwargs):
        ...
        self.hccl_comm_dict = kwargs.get("hccl_comm_dict", None)
        attn_bias = getattr(config, "bias", True)
        self.merged_qkv_proj = QKVParallelLinear(
            ...
            bias=attn_bias,
            return_bias=False,   # 不能开着，True的话输出是个Tuple (param, bias) 后面会报错
            skip_bias_add=False,
        ...
        self.enable_qk_norm = False
        if self.enable_qk_norm:
            self.q_norm = PanguEmbeddedRMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.k_norm = PanguEmbeddedRMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.batch_size = runner_settings.get("data_config").get("batch_size", 1)
    ...
    def exec_qkv(
        ...
        **kwargs,
        ...
        query_states = query_states.view(query_shape).contiguous()
        key_states = key_states.view(key_value_shape).contiguous()
...
ATTENTION_CLASSES = {
    "eager": PanguEmbeddedAttention,
}


class PanguEmbeddedDecoderLayer(nn.Module):
    def __init__(self, config: PanguEmbeddedConfig, runner_settings, layer_idx: int, prefix: str = "", **kwargs):
        super().__init__()
        ...
        self.mlp = PanguEmbeddedDenseMLP(
            config, runner_settings, prefix=f"{prefix}.mlp", **kwargs
        )
        self.input_layernorm = PanguEmbeddedRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = PanguEmbeddedRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.hccl_comm_dict = kwargs.get("hccl_comm_dict", None)
        self.batch_size = runner_settings.get("data_config").get("batch_size", 1)
...
PANGU_EMBEDDED_START_DOCSTRING = r"""
...
@add_start_docstrings(
    "The bare PanguEmbedded Model outputting raw hidden-states without any specific head on top.",
    PANGU_EMBEDDED_START_DOCSTRING,
)
...
PANGU_EMBEDDED_INPUTS_DOCSTRING = r"""
    Args:
...
class PanguEmbeddedModel(PanguEmbeddedPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`PanguEmbeddedDecoderLayer`]

    Args:
        config: PanguEmbeddedConfig
    """

    def __init__(self, config: PanguEmbeddedConfig, runner_settings, prefix: str = "", **kwargs):
        ...
        self.hccl_comm_dict = kwargs.get("hccl_comm_dict", None)
        # ===== Fused Layer 初始化 =====
        use_pto = bool(getattr(_pto_ops, "USE_PTO_FUSED_LAYER", False))
        self.use_pto_fused_layer = use_pto

        # 根据当前 rank 的 TP 配置调整参数
        num_kv_heads_per_rank = max(config.num_key_value_heads // self.attn_tp_size, 1)
        num_heads_per_rank = config.num_attention_heads // self.attn_tp_size
        ...
        if use_pto:
            ...
            fused_config = DynamicFusedLayerConfigV2BSH(
                hidden_size=config.hidden_size,
                intermediate_size=config.intermediate_size,
                num_attention_heads=num_heads_per_rank,
                num_key_value_heads=num_kv_heads_per_rank,
            ...
            self.fused_layers = nn.ModuleList(
                [
                    PanguFusedLayerV2BSHModule(fused_config)
    ...
    @add_start_docstrings_to_model_forward(PANGU_EMBEDDED_INPUTS_DOCSTRING)
    def forward(
        self,
        ...
        # Prefill (or PyPTO fused path disabled) -> standard PyTorch decoder layers.
        # The fused kernels are decode-only (batch=1, q_len=1), so multi-token
        # prefill and the fallback both run the un-fused ``PanguEmbeddedDecoderLayer``.
        if is_prefill or not self.use_pto_fused_layer:
            if self.attn_tp_size > 1 and self.attn_dp_size > 1:
            ...
            # ===== Decode: 走融合路径 =====
            # 准备 cos/sin: [max_seq_len, head_dim]
            cos = self.rotary_emb.cos_cached.contiguous()
            sin = self.rotary_emb.sin_cached.contiguous()

            # 初始化 residual: 首层为 zeros（_pypto_rms_norm 做 hidden+zeros=hidden，
            # 等价于 baseline 的 ln_npu(hidden_states)）
            residual = torch.zeros(1, 1, hidden_states.shape[-1],
                                   dtype=hidden_states.dtype, device=hidden_states.device)

            # ===== 单层融合模式：逐层调用 =====
            for layer_idx, decoder_layer in enumerate(self.layers):
                # --- KV cache: [B, max_seq, kv_dim] -> [max_seq, kv_dim] ---
                # 原始模型 cache: past_key_values[layer_idx] = (key_cache, value_cache)
                # key_cache: [B, max_seq, N_kv*D], batch=1 时取 [0] 得到 [max_seq, kv_dim]
                key_cache = past_key_values[layer_idx][0][0].contiguous()   # [max_seq, kv_dim]
                value_cache = past_key_values[layer_idx][1][0].contiguous()  # [max_seq, kv_dim]

                # --- 提取权重（pypto 要求 contiguous，NK 转置需 materialize）---
                input_ln_weight = decoder_layer.input_layernorm.weight
                post_ln_weight = decoder_layer.post_attention_layernorm.weight
                qkv_w = decoder_layer.self_attn.merged_qkv_proj.weight
                o_w = decoder_layer.self_attn.o_proj.weight
                gate_w = decoder_layer.mlp.gate_proj.weight
                up_w = decoder_layer.mlp.up_proj.weight
                down_w = decoder_layer.mlp.down_proj.weight
                qkv_weight = qkv_w if qkv_w.is_contiguous() else qkv_w.t().contiguous()
                o_weight = o_w if o_w.is_contiguous() else o_w.t().contiguous()
                gate_weight = gate_w if gate_w.is_contiguous() else gate_w.t().contiguous()
                up_weight = up_w if up_w.is_contiguous() else up_w.t().contiguous()
                down_weight = down_w if down_w.is_contiguous() else down_w.t().contiguous()
                qkv_bias = decoder_layer.self_attn.merged_qkv_proj.bias.contiguous()
                o_bias = decoder_layer.self_attn.o_proj.bias.contiguous()

                # --- 准备输入 ---
                # hidden_states: [B, 1, hidden_size] -> [1, 1, hidden_size]
                if hidden_states.dim() == 3 and hidden_states.shape[0] == 1:
                    hidden_3d = hidden_states
                elif hidden_states.dim() == 2:
                    hidden_3d = hidden_states.unsqueeze(0)  # [1, 1, hidden_size]
                else:
                    hidden_3d = hidden_states  # 已经是 [1, 1, hidden_size]
                # --- 调用融合 kernel ---
                fused_layer = self.fused_layers[layer_idx]
                output, new_residual = fused_layer(
                ...
                # 传递给下一层
                hidden_states = output      # [1, 1, hidden_size]
                residual = new_residual      # [1, 1, hidden_size]
...
class PanguEmbeddedForCausalLM(PanguEmbeddedPreTrainedModel):
    ...
    def __init__(self, config, runner_settings, prefix: str = ""):
        ...
        self.model = PanguEmbeddedModel(config, runner_settings, prefix, **kwargs)
        ...
        # Initialize weights and apply final processing
        self.post_init()
        input_max_len = runner_settings["data_config"]["input_max_len"]
        max_new_tokens = runner_settings["data_config"]["max_new_tokens"]
        logger.info(
            "check: input_max_len(%s) + max_new_tokens(%s) = %s",
            input_max_len,
            max_new_tokens,
            input_max_len + max_new_tokens,
        )
    ...
    def get_parallel_settings(self):
        ...
        self.dense_tp_size = self.runner_settings.get("parallel_config").get("dense_tp_size", self.attn_tp_size)
        self.dense_dp_size = self.runner_settings.get("parallel_config").get("dense_dp_size", 1)
    ...
    def init_parallel_comm_group(self):
        ...
        if self.dense_tp_size == self.attn_tp_size:
            ...
            dense_tp_group = init_comm_group(
                global_rank=global_rank, group_num=self.dense_dp_size, world_size=world_size,
    ...
    def gen_cur_topk_idx(
        ...
        if (
            not self.perfect_eplb
            or not hasattr(self.config, "num_experts")
            or not hasattr(self.config, "num_experts_per_tok")
        ):
            return None
        global_rank = dist.get_rank()
        if is_prefill:
            if self.moe_ep_size != 1:
                tokens_per_rank_prefill = (
                    batch_size * seq_len + self.attn_tp_size - 1
                ) // self.attn_tp_size
            else:
            ...
            cur_topk_list = (
                torch.tensor(cur_topk_list_prefill)
                .int()
                .view(tokens_per_rank_prefill, -1)
                .npu()
            )
        else:
            if self.moe_tp_size > 1:
                ...
                expanded_tokens = batch_size * self.config.num_experts_per_tok * seq_len
                if expanded_tokens < self.config.num_experts:
                    step_gap = self.config.num_experts // self.moe_ep_size
                else:
                    step_gap = 1

                rank_step = global_rank // step_gap * step_gap
                rank_mod = global_rank % step_gap
                cur_topk_list_decode = []
                for i in range(expanded_tokens):
                    expert_idx = ((i + rank_step) * step_gap + rank_mod) % self.config.num_experts
                    cur_topk_list_decode.append(expert_idx)
                cur_topk_list = torch.tensor(cur_topk_list_decode).int().view(batch_size * seq_len, -1).npu()
        return cur_topk_list

    # Adapted from vllm.model_executor.models.openpangu.PanguEmbeddedModel.load_weights
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        logger.debug("params_dict keys preview: %s", list(params_dict.keys())[:10])
        loaded_params: set[str] = set()
        # Dense model compatibility: make sure variable exists
        ...
        for name, loaded_weight in weights:
            original_name = name
            if "rotary_emb.inv_freq" in name and name not in params_dict:
                continue
            if "mlp.experts." in name and not hasattr(self.config, "num_experts"):
                expert_tokens = name.split(".mlp.experts.")
                if len(expert_tokens) == 2 and expert_tokens[1].startswith("0."):
                    name = f"{expert_tokens[0]}.mlp.{expert_tokens[1][2:]}"
                else:
                    continue
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                # name will be updated and then remapped again below, which
                # breaks expert weight loading.
                ...
                for mapping in expert_params_mapping:
                    ...
                    if name not in params_dict:
                        logger.debug(
                            "[MISSING WEIGHT]: '%s' not found in params_dict keys preview=%s. Skipping...",
                            name,
                            list(params_dict.keys())[:10],
                        )
                        continue
                    param = params_dict[name]
                    weight_loader = getattr(param, "weight_loader",
