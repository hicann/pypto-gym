...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # PyPTO fused Q/K RMSNorm + RoPE (prefill only; decode falls back to eager)
        import sys
        pto_kernels = sys.modules.get("qwen3_pto_kernels")
        if pto_kernels is not None and pto_kernels.USE_PTO_ROPE and hidden_states.shape[1] > 1:
            query_states, key_states = pto_kernels.qk_rope_wrapper(
                self.q_proj(hidden_states), self.k_proj(hidden_states),
                cos, sin,
                self.q_norm.weight, self.k_norm.weight,
                self.config.num_attention_heads, self.config.num_key_value_heads, self.head_dim
            )
