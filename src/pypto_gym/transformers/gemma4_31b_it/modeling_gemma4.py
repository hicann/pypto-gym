...
class Gemma4AudioAttention(nn.Module):
    ...
    def __init__(self, config: Gemma4AudioConfig, layer_idx: int):
        ...
        self.q_proj = Gemma4ClippableLinear(
            config, config.hidden_size, self.num_heads * self.head_dim
        )
...
class Gemma4AudioSubSampleConvProjectionLayer(nn.Module):
    def __init__(self, in_channels, out_channels, norm_eps):
        ...
        self.norm = nn.LayerNorm(
            out_channels, eps=norm_eps, elementwise_affine=True, bias=False
        )
        self.act = nn.ReLU()
...
class Gemma4AudioFeedForward(nn.Module):
    def __init__(self, config: Gemma4AudioConfig):
        ...
        self.ffw_layer_1 = Gemma4ClippableLinear(
            config, config.hidden_size, config.hidden_size * 4
        )
        self.ffw_layer_2 = Gemma4ClippableLinear(
            config, config.hidden_size * 4, config.hidden_size
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = torch.clamp(
            hidden_states, -gradient_clipping, gradient_clipping
        )
...
class Gemma4AudioLightConv1d(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.depthwise_conv1d(hidden_states.transpose(1, 2)).transpose(
            1, 2
        )
...
class Gemma4AudioLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # This is needed to avoid any underflow/overflow issues when clipping
        gradient_clipping = min(
            self.gradient_clipping, torch.finfo(self.norm_pre_attn.weight.dtype).max
        )
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    # upcast attention to fp32 — PYPTO_PATCH: use tiled PyPTO softmax when switch on
    _pk = sys.modules.get("gemma4_pto_kernels")
    if _pk is not None and getattr(_pk, "USE_PTO_SOFTMAX", False):
        B, N, Sq, Sk = attn_weights.shape
        attn_weights = _pk.attn_softmax(attn_weights.reshape(-1, Sk), scale=1.0).view(
            B, N, Sq, Sk
        )
    else:
...
class Gemma4VisionAttention(nn.Module):
    ...
    def __init__(self, config: Gemma4VisionConfig, layer_idx: int):
        ...
        self.q_proj = Gemma4ClippableLinear(
            config, config.hidden_size, config.num_attention_heads * self.head_dim
        )
        self.k_proj = Gemma4ClippableLinear(
            config, config.hidden_size, config.num_key_value_heads * self.head_dim
        )
        self.v_proj = Gemma4ClippableLinear(
            config, config.hidden_size, config.num_key_value_heads * self.head_dim
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if (_pk is not None and getattr(_pk, "USE_PTO_GQA", False)
            ...
            layer_kind = "local" if self.is_sliding else "global"
            attn_output = _pk.gqa_decode_attn(
                query_states, key_states, value_states, attention_mask,
                self.scaling, layer_kind
            )
            attn_weights = None
        else:
            attention_interface: Callable = ALL_ATTENTION_FUNCTIONS.get_interface(
...
class Gemma4PreTrainedModel(PreTrainedModel):
        ...
        if isinstance(module, Gemma4VisionPatchEmbedder):
            ...
            for layer_type, rope_init_fn in module.rope_init_fns.items():
                ...
                if (
                    ...
                    and module.rope_type[layer_type] == "proportional"
                ):
                    rope_init_fn_kwargs["head_dim_key"] = "global_head_dim"
    ...
    def resize_token_embeddings(
        ...
        self._resize_per_layer_embeddings(
    ...
    def _resize_per_layer_embeddings(
        ...
        if self.config.get_text_config().hidden_size_per_layer_input:
            ...
            new_embeddings_per_layer.requires_grad_(
                embed_tokens_per_layer.weight.requires_grad
            )
            self.set_per_layer_input_embeddings(new_embeddings_per_layer)


@auto_docstring(
    custom_intro="The base Gemma 4 language model without a language modeling head."
)
...
@auto_docstring(
    custom_intro="The base Gemma 4 language model with a language modeling head."
)
...
class Gemma4AudioModel(Gemma4PreTrainedModel):
    ...
    def __init__(self, config: Gemma4AudioConfig):
        ...
        self.layers = nn.ModuleList(
            [
                Gemma4AudioLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Gemma4AudioModelOutput(
            last_hidden_state=hidden_states, attention_mask=output_mask
        )
...
class Gemma4MultimodalEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self.multimodal_hidden_size = getattr(
            multimodal_config, "output_proj_dims", multimodal_config.hidden_size
        )
...
class Gemma4Model(Gemma4PreTrainedModel):
    ...
    def __init__(self, config: Gemma4Config):
        ...
        self._keys_to_ignore_on_load_unexpected = [
            ...
            for name in self.language_model._keys_to_ignore_on_load_unexpected
        ]
        self.post_init()
    ...
    @can_return_tuple
    @auto_docstring(
        custom_intro="Projects the last hidden state from the vision model into language model space."
    )
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values_videos is not None:
            ...
            inputs_embeds = inputs_embeds.masked_scatter(
                ...
                video_features.to(inputs_embeds.device),
            )
    ...
    @can_return_tuple
    @auto_docstring(
        custom_intro="Projects the last hidden state from the audio encoder into language model space."
    )
    def get_audio_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        audio_outputs.pooler_output = self.embed_audio(
            inputs_embeds=audio_outputs.last_hidden_state
        )
    ...
    @can_return_tuple
    @auto_docstring(
        custom_intro="Projects the last hidden state from the vision encoder into language model space."
    )
...
class Gemma4ForConditionalGeneration(Gemma4PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if (
            ...
            := self.config.get_text_config().final_logit_softcapping
        ) is not None:
        ...
        if labels is not None:
            loss = self.loss_function(
                logits, labels, self.config.get_text_config().vocab_size, **kwargs
            )
    ...
    def create_masks_for_generate(
        ...
        **kwargs,
        ...
        if (
            getattr(config.get_text_config(), "use_bidirectional_attention", None)
            == "vision"
        ):
            # Larger Gemma 4 models use Gemma 3's bidirectional attention mask for vision inputs
