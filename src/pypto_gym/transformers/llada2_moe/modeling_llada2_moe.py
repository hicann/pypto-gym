...
class LLaDA2MoeSparseMoeBlock(nn.Module):
    ...
    def _ensure_pypto_weights(self):
        """Lazily stack expert weights for PyPTO kernels (cached as buffers)."""
        if getattr(self, "_pypto_ready", False):
            return
        ...
        if self.config.num_shared_experts is not None:
            ...
            self.register_buffer(
                "_pypto_shared_w13",
                torch.cat([se.gate_proj.weight.t(),
                           se.up_proj.weight.t()], dim=1).contiguous(),
                persistent=False,
            )
            self.register_buffer(
                "_pypto_shared_w2",
                se.down_proj.weight.t().contiguous(),
                persistent=False,
            )
        if self.gate.weight.dtype != torch.float32:
            self.gate.weight.data = self.gate.weight.data.to(torch.float32)
        if self.gate.expert_bias.dtype != torch.float32:
            self.gate.expert_bias.data = self.gate.expert_bias.data.to(torch.float32)
        self._pypto_ready = True

    @torch.no_grad()
    def moe_infer(self, x, topk_ids, topk_weight):
        # PYPTO_PATCH: use PyPTO expert_ffn kernel when switch is on
        _pk = sys.modules.get("llada2_pto_kernels")
        if _pk is not None and getattr(_pk, "USE_PTO_EXPERT_FFN", False):
            return self._moe_infer_pypto(x, topk_ids, topk_weight)
    ...
    @torch.no_grad()
    def _moe_infer_pypto(self, x, topk_ids, topk_weight):
        """PyPTO-accelerated MoE inference with grouped GEMM (single kernel call)."""
        _pk = sys.modules.get("llada2_pto_kernels")
        grouped_gemm = _pk.grouped_gemm

        self._ensure_pypto_weights()
        N = x.shape[0]
        K = topk_ids.shape[1]
        E = len(self.experts)
        H = x.shape[1]
        I = self._pypto_w2_stack.shape[1]

        flat_ids = topk_ids.view(-1).to(torch.int64)
        sort_perm = flat_ids.argsort()
        sorted_ids = flat_ids[sort_perm]
        tok_src = sort_perm // K
        sorted_x = x[tok_src].contiguous()

        # Build cumsum for grouped_gemm
        counts = torch.bincount(sorted_ids, minlength=E)
        cumsum = torch.zeros(E + 1, dtype=torch.int32, device=x.device)
        cumsum[1:] = torch.cumsum(counts, 0).to(torch.int32)

        # Flatten stacked weights: [E, H, 2I] -> [E*H, 2I], [E, I, H] -> [E*I, H]
