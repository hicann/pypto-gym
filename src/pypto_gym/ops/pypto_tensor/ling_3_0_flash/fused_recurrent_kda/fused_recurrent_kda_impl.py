    ...
    **kwargs,
    ...
    if ssm_state_indices is not None and ssm_state_indices.numel() > 0:
        if ssm_state_indices.ndim == 1:
            ssm32 = ssm_state_indices.contiguous().unsqueeze(-1)  # [N] -> [N, 1]
        else:
            ssm32 = ssm_state_indices.contiguous()  # [N, T] as-is
    else:
    ...
    if is_spec_decoding:
        nat32 = num_accepted_tokens.contiguous()
    else:
