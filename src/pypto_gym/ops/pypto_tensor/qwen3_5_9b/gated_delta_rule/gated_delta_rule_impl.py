...
# ---------------------------------------------------------------------------
# `@allow_in_graph` lets dynamo treat the wrapper as opaque; graph capture
# additionally needs a Meta ("fake") impl for shape/dtype inference without
# launching the kernel. Registering `pypto::gated_delta_rule_qwen3_5` (Meta + NPU)
# provides that; reached via `gated_delta_rule_pypto`. The wrapper has no in-kernel
# synchronize (stream-ordered), so the registered op is aclgraph-capture-clean.
# Mirrors the kimi_linear KDA registration. output_final_state is fixed True here.
pyptolib = torch.library.Library("pypto", "FRAGMENT")  # type: ignore[arg-type]

if not hasattr(torch.ops.pypto, "gated_delta_rule_qwen3_5"):
    pyptolib.define(
        "gated_delta_rule_qwen3_5(Tensor query, Tensor key, Tensor value, "
        "Tensor g, Tensor beta, Tensor? initial_state, bool use_qk_l2norm_in_kernel) "
        "-> (Tensor, Tensor)")

    @torch.library.impl(pyptolib, "gated_delta_rule_qwen3_5", "Meta")  # type: ignore[arg-type]
    def _gdr_qwen3_5_meta(query, key, value, g, beta, initial_state, use_qk_l2norm_in_kernel):
        b, s, nv, d = query.shape
    ...
    @torch.library.impl(pyptolib, "gated_delta_rule_qwen3_5", "NPU")  # type: ignore[arg-type]
    def _gdr_qwen3_5_npu(query, key, value, g, beta, initial_state, use_qk_l2norm_in_kernel):
...
def gated_delta_rule_pypto(query, key, value, *, g, beta, initial_state=None,
                           use_qk_l2norm_in_kernel=True):
    """aclgraph-capturable entry: routes through ``torch.ops.pypto.gated_delta_rule_qwen3_5``.
    ...
    """
    return torch.ops.pypto.gated_delta_rule_qwen3_5(
