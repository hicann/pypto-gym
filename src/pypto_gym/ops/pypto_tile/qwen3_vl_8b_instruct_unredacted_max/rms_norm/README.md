...
将 RMSNorm 算子从原始 torch 实现替换为 torch_npu.npu_rms_norm，提升 NPU 上的推理性能。
...
```bash
python3 ask_Qwen3-1.7B.py --prompt "你好" --use-pto
```

### 禁用 PTO（使用原始 torch 实现）

```bash
python3 ask_Qwen3-1.7B.py --prompt "你好"
```

## 测试验证

```bash
cd /mnt/workspace/gitCode/cann/models/Qwen3-1.7B
...
最终使用 `torch_npu.npu_rms_norm` 而非 PyPTO 自定义实现，原因：
- torch_npu.npu_rms_norm 是已验证的 NPU 算子
- 精度测试通过（max diff < 4e-3）
- 性能优于自定义实现
