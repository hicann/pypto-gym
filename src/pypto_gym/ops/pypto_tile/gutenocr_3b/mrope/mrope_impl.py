...
def mrope_pto_correct(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    try:
        ...
        with pypto.function("MROPE_KERNEL", q_tensor, k_tensor, cos_tensor, sin_tensor, q_output, k_output):
            ...
            for block_idx in pypto.loop(num_blocks, name="LOOP_MROPE", idx_name="block_idx"):
                # 创建临时 tensor
...
def mrope_torch_fallback(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    cos_new = torch.cat([m[i % 3] for i, m in enumerate(cos.split(mrope_section_expanded, dim=-1))], dim=-1).unsqueeze(unsqueeze_dim)
    sin_new = torch.cat([m[i % 3] for i, m in enumerate(sin.split(mrope_section_expanded, dim=-1))], dim=-1).unsqueeze(unsqueeze_dim)
