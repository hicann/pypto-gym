...
"""gated_delta_rule — chunked multi-head linear-attention kernel for Qwen3.6-27B.
...
_BN = _NV         # B=1, Nv=48  →  BN = 48 (constant, enforced by wrapper)
_BND = _BN * _D   # 48 * 128 = 6144 (constant)
...
def _gdr_kernel(
    ...
    for bn_idx in pypto.loop(BN, name="bn_loop", idx_name="bn_idx", parallel=True):
        ...
        for s_idx in pypto.loop(0, S_pad, L, name="s_loop", idx_name="s_idx", unroll_list=[8, 1]):
...
def gated_delta_rule_wrapper(
    ...
    *,
    ...
    """Chunk gated delta rule fused kernel for Qwen3.6-27B prefill.
