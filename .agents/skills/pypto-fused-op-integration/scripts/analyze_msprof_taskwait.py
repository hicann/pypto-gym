...
def _print_global_stats(prefix, total_wait, total_dur, op_stats, n_rows):
    ...
    print(f"{'OP Type':28s} {'Count':>7s} {'Wait(s)':>8s} {'Dur(s)':>8s} {'AvgWait':>8s}")
    print("-" * 62)
...
def main():
    ...
    if len(all_seg_stats) >= 2:
        print(f"\n{'=' * 70}")
        print("对比汇总")
        print(f"{'标签':>20s} {'每层任务':>8s} {'计算(us)':>9s} {'avgWait':>8s} {'medWait':>8s}")
        print("-" * 62)
        for label, segs in all_seg_stats:
            ...
            print(f"{label:>20s} {avg_tasks:>8.0f} {avg_dur:>9.1f} {avg_w:>8.1f}us {med_w:>8.1f}us")
