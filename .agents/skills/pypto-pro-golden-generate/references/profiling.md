...
```bash
python3 .agents/skills/pypto-pro-golden-generate/scripts/profile_golden.py \
...
- 若 torch_npu 未安装，profiling 阶段返回 `npu_import_error`，引导调用 `pypto-environment-setup`（注：pypto-pro 与 pypto 安装方式完全一致——标准安装带 pro 框架代码的 pypto 后 pypto-pro 自动安装，因此环境安装统一引用 `pypto-environment-setup`，无需单独的 pro 版环境安装 skill）
