...
- 与 `pypto-environment-setup`（PyPTO 非 Pro 的安装/修复重型 skill）职责不同：本 skill 面向 Pro 工作流、纯检测。若检测判定需要安装/修复，**引导**调用者转向 `pypto-environment-setup`，本 skill 不代劳。
...
timeout 300 python .agents/skills/pypto-pro-environment-check/scripts/test_matmul_8k_example.py
```

该测试的行为（`.agents/skills/pypto-pro-environment-check/scripts/test_matmul_8k_example.py`）：
...
```bash
python .agents/skills/pypto-pro-environment-check/scripts/env_preflight.py
```
...
| [3] 运行时 | `torch_npu 未安装` | 引导用户使用 `pypto-environment-setup` skill 安装 torch_npu（本 skill 不代装） |
| [3] 运行时 | `torch_npu 已安装但初始化失败` | 多为版本不配套或环境变量缺失；查 CANN 与 torch_npu 版本配套，引导 `pypto-environment-setup` |
| [3] 运行时 | `import pypto_pro.language 失败` | pypto_pro 未安装 / 未编译；引导 `pypto-environment-setup` 编译安装 PyPTO-Pro |
...
3. **需要修复时引导转向**：判定需安装/修复时，明确建议调用者加载 `pypto-environment-setup`。
