...
#
# 用法：
#   python .agents/skills/pypto-pro-environment-check/scripts/env_preflight.py          # 完整检测
#   python .agents/skills/pypto-pro-environment-check/scripts/env_preflight.py --json    # 仅输出 JSON
# -----------------------------------------------------------------------------
...
def _check_devices_via_torch_npu(rep: Report) -> bool:
    ...
    for i in range(count):
        # 设备名：get_device_name 是标准 torch 设备 API，防御式调用
        try:
        ...
        dev = {
            "npu_id": i,
            "chip_name": name,
            # 部分环境 npu-smi 是残缺 stub、无逐卡健康位，torch_npu 也无逐卡
            # 健康 API；以 is_available() 为准，如实标注来源，不臆测 stub 行格式。
            "health": "available",
            "health_source": "torch_npu.is_available",
        }

        # 规格字段：仅在 get_device_properties 提供时记录，缺失不报错
        try:
            props = torch.npu.get_device_properties(i)
            for attr in ("total_memory", "cube_core_num",
                         "vector_core_num", "L2_cache_size", "uuid"):
                val = getattr(props, attr, None)
                if val is not None:
                    dev[attr] = str(val)
        except Exception as e:
            logging.debug("get_device_properties failed for NPU %d: %s", i, e)

        rep.devices.append(dev)
        rep.success(f"  NPU {i}: {name} | 可用 (via torch_npu.is_available)")
...
def check_devices(rep: Report):
    ...
    try:
        ...
        from _npu_info import NpuInfoCollector  # type: ignore
...
def check_runtime(rep: Report):
    ...
    try:
        import pypto_pro.language as pl  # type: ignore
        ...
        if hasattr(pl, "jit"):
            ...
            rep.warning("pypto_pro.language 无 jit 属性")
    except ImportError as e:
...
def check_cann(rep: Report):
    ...
    # 从 ASCEND_OPP_PATH 推导（最可靠：指向实际使用的 toolkit）
    opp = os.environ.get("ASCEND_OPP_PATH", "")
    if opp.endswith("/opp"):
        cand = opp[:-4]
        if os.path.isdir(os.path.join(cand, "compiler")):
            toolkit = cand

    if not toolkit:
        for var in ("ASCEND_TOOLKIT_HOME", "ASCEND_HOME", "ASCEND_HOME_PATH"):
            v = os.environ.get(var, "")
            resolved = _resolve_toolkit_path(v)
            if resolved:
                toolkit = resolved
                break
    ...
    # 版本
    ver_info = os.path.join(toolkit, "compiler", "version.info")
    version = None
    if os.path.isfile(ver_info):
        try:
            with open(ver_info) as f:
                for line in f:
                    if line.startswith("Version="):
                        version = line.split("=", 1)[1].strip()
                        break
        except Exception as e:
            logging.debug("Failed to read CANN version.info: %s", e)
    if not version:
        version = os.path.basename(toolkit).replace("cann-", "")
...
def main():
    ...
    # 末尾一行机器可读 JSON
    LOGGER.info("PREFLIGHT_JSON: " + json.dumps(rep.to_dict(), ensure_ascii=False))
