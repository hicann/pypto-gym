...
从 `GOLDEN_PERF_REPORT.md` 读取 golden 标杆性能数据（由 `pypto-pro-golden-generate` skill 的 `profile_golden.py` 采集，含 E2E kernel duration），同时用 msprof 采集 PyPTO 算子的 kernel Task Duration，计算加速比：

```bash
bash scripts/msprof_profile_run.sh --compare --output-dir=./custom/{op} --warm-up=3 --device=0 --op-name=<Op Name>
```
...
对 pypto-pro 测试脚本做快速采集，只跑 1 轮 msprof，不采集 7 个 aic-metrics：

```bash
bash scripts/msprof_profile_run.sh --quick --output-dir=./custom/{op} --warm-up=3 --device=0 --op-name=<Op Name>
```
