...
def generate_summary(opprof_dir: str, round_dir: str, ops_dir: str) -> str:
    ...
    if basic_rows:
        ...
        lines.append(
            f"Op: {op_name} | Type: {op_type} | Duration: {duration}us | "
            f"BlockDim: {block_dim} | Freq: {cur_freq:.0f}/{rated_freq:.0f}"
        )
    else:
        lines.append("=== 上板性能统计摘要 ===")
    ...
    if pipe_rows:
        ...
        if duration > 0 and core_times:
            ...
            lines.append(
                ...
                f"头开销: {overhead:.2f}us ({overhead_pct:.1f}%)"
            )
    ...
    if mem_rows:
        ...
        if sum(gm_ub) > 0:
            ...
            lines.append(
                f"  GM→UB: {sum(gm_ub):.1f}KB total "
                f"({statistics.mean(gm_ub):.1f}KB/core), BW usage: {statistics.mean(bw_usage):.2f}%"
            )
        if sum(ub_gm) > 0:
            bw_usage = [safe_float(r.get("UB_to_GM_bw_usage_rate(%)")) for r in mem_rows]
            lines.append(
                f"  UB→GM: {sum(ub_gm):.1f}KB total "
                f"({statistics.mean(ub_gm):.1f}KB/core), BW usage: {statistics.mean(bw_usage):.2f}%"
            )
        if sum(gm_l1) > 0:
            bw_usage = [safe_float(r.get("GM_to_L1_bw_usage_rate(%)")) for r in mem_rows]
            lines.append(
                f"  GM→L1: {sum(gm_l1):.1f}KB total "
                f"({statistics.mean(gm_l1):.1f}KB/core), BW usage: {statistics.mean(bw_usage):.2f}%"
            )
        if sum(read_mm) > 0:
        ...
        mte2_instr = sum(
            safe_float(r.get(f"{prefix}_mte2_instructions")) +
            safe_float(r.get("aic_mte2_instructions" if prefix == "aiv" else "aiv_mte2_instructions"))
            for r in mem_rows
        )
        # Simpler: just sum both
        mte2_instr_total = sum(
            safe_float(r.get("aiv_mte2_instructions", 0)) +
            safe_float(r.get("aic_mte2_instructions", 0))
            for r in mem_rows
        )
        if mte2_instr_total > 0 and sum(gm_ub) > 0:
            avg_transfer = sum(gm_ub) * 1024 / mte2_instr_total
            lines.append(
                f"  Avg MTE2 transfer: {avg_transfer / 1024:.2f}KB "
                f"({int(mte2_instr_total)} instructions total)"
            )
    ...
    if l2_rows:
        ...
        for rate_name, field in [("total_hit", f"{prefix}_total_hit_rate(%)"),
            ...
            if any(v > 0 for v in vals):
                lines.append(
                    f"  {rate_name}: avg={statistics.mean(vals):.1f}% "
