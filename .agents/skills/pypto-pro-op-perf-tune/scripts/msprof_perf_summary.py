...
def _parse_golden_report(out_dir: Path):
    ...
    返回 (cases, error)，cases = [(case_name, shape_str, dtype_str, e2e_us), ...]
    """
        ...
        if in_summary_table:
            if line.startswith("|") and "---" not in line:
                ...
                if len(parts) >= 5:
                    ...
                    shape = parts[2]
                    dtype = parts[3]
                    e2e_str = parts[4].replace("us", "").strip()
                    try:
                        e2e_us = float(e2e_str)
                        cases.append((case_name, shape, dtype, e2e_us))
                    except ValueError:
                        pass
    ...
    if not cases:
        ...
        for line in content.splitlines():
            ...
            if line.startswith("- **Input Shape**:"):
                ...
                if m:
                    e2e_us = float(m.group(1))
        if e2e_us is not None:
            cases.append(("P0", shape, dtype, e2e_us))
...
def _run_msprof_standard(test_script: str, output_dir: str, warmup: int = 3, device_id: int = 0):
...
def _run_msprof_quick(test_script: str, output_dir: str, warmup: int = 3, device_id: int = 0):
...
def _parse_msprof_duration_quick(prof_group_dir: str, op_name: str = None):
    ...
    指定 op_name 时，只匹配该 kernel_name 的行（避免选到 torch.randn 等非目标 op）。
    """
    ...
    if task_time_files:
        try:
            ...
            if kernel_times:
                if op_name:
                    if op_name in kernel_times:
                        main_kernel = op_name
                        times = kernel_times[op_name]
                    else:
                    ...
                    times = kernel_times.get(main_kernel, [])
                if len(times) >= 3:
...
def _measure_pypto(out_dir: Path, args, device_id: int):
    """Measure PyPTO operator by running test_{op}.py with msprof (standard mode).
    ...
    for _ in range(1 + args.retry):
        tmpdir = f"/tmp/msprof_pypto_{out_dir.name}"
        prof_dir, err = _run_msprof_standard(test_script, tmpdir, args.warmup, device_id)
        if prof_dir:
            duration, _op_name, parse_err = _parse_msprof_duration(prof_dir, getattr(args, 'op_name', None))
...
def _measure_pypto_quick(out_dir: Path, args, device_id: int):
    """快速模式：Measure PyPTO operator with retries and repeats.
    ...
    for _ in range(1 + args.retry):
        ...
        for r in range(repeats):
            tmpdir = f"/tmp/msprof_quick_pypto_{out_dir.name}_r{r}"
            _cleanup_prof_dirs(tmpdir)
            prof_dir, err = _run_msprof_quick(test_script, tmpdir, args.warmup, device_id)
            if not prof_dir:
                break

            duration, _op_name, parse_err = _parse_msprof_duration_quick(prof_dir, getattr(args, 'op_name', None))
            if duration is None:
                err = parse_err
                break
...
def _run_compare_loop(out_dir, golden_cases, args, device_id):
    """Run the measurement loop over all golden cases. Returns rows and stats.

    golden_cases: [(case_name, shape, dtype, e2e_us), ...] from GOLDEN_PERF_REPORT.md
    PyPTO 算子通过 msprof 采集 test_{op}.py 一次，取 op_summary 最大 Task Duration。
    """
    rows, speedups, ref_times, asc_times = [], [], [], []

    pypto_us, pypto_err, pypto_prof_dir = _measure_pypto(out_dir, args, device_id)

    for case_name, golden_shape, golden_dtype, golden_us in golden_cases:
        if pypto_us is not None and pypto_us > 0:
            sp = golden_us / pypto_us
            speedups.append(sp)
            ref_times.append(golden_us)
            asc_times.append(pypto_us)
            LOGGER.info(f"{case_name:<20} {golden_shape:<35} {golden_dtype:<10} "
                  f"{golden_us:>12.2f} {pypto_us:>12.2f} {sp:>9.3f}x")
        else:
            LOGGER.info(f"{case_name:<20} {golden_shape:<35} {golden_dtype:<10} "
...
def _run_quick_loop(out_dir, golden_cases, args, device_id):
    """快速模式：Run the measurement loop (只跑 1 轮 msprof).

    golden_cases: [(case_name, shape, dtype, e2e_us), ...] from GOLDEN_PERF_REPORT.md
    """
    rows, speedups, ref_times, asc_times = [], [], [], []

    pypto_us, pypto_err, pypto_prof_dir = _measure_pypto_quick(out_dir, args, device_id)
...
def run_compare_mode(args):
    ...
    golden_cases, golden_err = _parse_golden_report(out_dir)
    if not golden_cases:
        LOGGER.error(f"[ERROR] Failed to load golden data: {golden_err}")
        return 1
    LOGGER.info(f"[INFO] Loaded {len(golden_cases)} golden cases from GOLDEN_PERF_REPORT.md")
...
def run_quick_mode(args):
    """执行快速模式：test_{op}.py（只跑 1 轮 msprof，不采集 7 个 metrics）"""
    out_dir = Path(args.output_dir).resolve()
