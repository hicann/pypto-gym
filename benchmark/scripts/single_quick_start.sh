#!/usr/bin/env bash
# One-command quick start for a single benchmark case.

set -euo pipefail

if [ "$#" -ne 0 ]; then
  echo "ERROR: single_quick_start.sh does not accept arguments." >&2
  echo "Usage: bash benchmark/scripts/single_quick_start.sh" >&2
  exit 2
fi
