#!/usr/bin/env python3
"""
Phi-3-mini-4k-instruct 推理脚本 (with benchmark instrumentation)
用法: python3 ask_Phi-3-mini-4k-instruct.py [--prompt "问题"] [--device 卡号] [--model-path 路径]
...
parser.add_argument("--use-acl-graph", action="store_true", help="启用 ACL Graph (JIT Compile 模式)")
...
# ---- PyPTO setup (must be before transformers import) ----
if args.use_pypto or args.use_acl_graph:
...
mode_label = []
if args.use_acl_graph:
    mode_label.append("aclgraph")
if args.use_pypto:
    mode_label.append("pypto")
metrics["mode"] = "+".join(mode_label) if mode_label else "baseline"
