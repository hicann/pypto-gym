# Qwen3.6-27B NPU 迁移说明

| 字段 | 说明 |
|------|------|
| HuggingFace | Qwen/Qwen3.6-27B |
| 权重目录 | 由 `MODEL_PATH` 环境变量 / `--model-path` 指定（Qwen3.6-27B 权重目录） |
| 代码来源 | transformers 包 (built-in，使用 qwen3_5 架构) |
| 运行命令 | `python3 ask_Qwen3.6-27B.py --model-path <weights>` |
| transformers 版本 | 5.8.1（实测运行 / 与上方环境信息一致） |
| 代码位置 | 运行时：transformers 内置 `transformers.models.qwen3_5`；归档快照：`src/pypto_gym/transformers/qwen3_6_27b/{modeling,configuration}_qwen3_5.py` |
| 修改内容 | 运行时由 `ask_Qwen3.6-27B.py` 经 `sys.modules` 注入 + monkey-patch 接入 PyPTO（built-in `qwen3_5` 架构，`config.json` 无 auto_map）。归档快照在内置 qwen3_5 modeling 中嵌入同一 chunk 注入钩子（含 `query.device.type == "npu"` 守卫）+ 华为 NOTICE，供参考；保留上游相对导入，未改绝对导入 |
...
`--use_pypto` 模式依赖运行时 `qwen3_6_27b_pto_kernels/` 适配层包：脚本在导入
`transformers` 之前 `sys.path.insert(0, model_path)` 并 `import
qwen3_6_27b_pto_kernels`。该适配层包预期部署在权重目录下
（`{weights_dir}/qwen3_6_27b_pto_kernels/`），由模型侧维护。

## 运行

```bash
# Baseline
python3 ask_Qwen3.6-27B.py --model-path <weights_dir>

# PyPTO fused
python3 ask_Qwen3.6-27B.py --model-path <weights_dir> --use_pypto
```
...
python3 bench_qwen3_6_27b.py --model-path <weights> [--use_pypto]
bash bench_qwen3_6_27b.sh   # eager + PyPTO 两阶段, 写 bench_baseline.json / bench_pypto.json
```

> ℹ️ `bench_qwen3_6_27b.py` 测的是**可部署**的整网 greedy 生成（真实模型、**无图捕获**），
...
（`bench_qwen3_6_27b.py`，output=100，1 次 warmup + 1 次 measured generate，单卡 910B3；实测环境见「环境信息」）。
...
|------|------|----------------------|----------------|------|---------|
| 自然 prompt (~16 tok) | eager（torch chunk 回退） | 811.5 | 4.99 | 1.00x | ~52 GB |
| 自然 prompt (~16 tok) | **PyPTO**（融合 kernel） | **209.7** | 5.02 | **3.87x** | ~52 GB |
| 256-token 输入 | eager（torch chunk 回退） | 904.1 | 5.11 | 1.00x | ~52 GB |
| 256-token 输入 | **PyPTO**（融合 kernel） | **212.5** | 4.83 | **4.25x** | ~52 GB |
...
> chunk 路径，故首 token 延迟大幅下降（自然 prompt 3.87x、256-token 输入 4.25x）。**decode（27B ~5 tok/s）保持不变**——
...
|---|---|
| `ask_Qwen3.6-27B.py`, `bench_qwen3_6_27b.py`, `bench_qwen3_6_27b.sh`, `README.md` | `modeling/transformers/qwen3_6_27b/` |
| `modeling_qwen3_5.py`, `configuration_qwen3_5.py`（带 PyPTO 钩子的内置 qwen3_5 归档快照）, `README.md` | `src/pypto_gym/transformers/qwen3_6_27b/` |
| `qwen3_6_27b_pto_kernels/gated_delta_rule/gated_delta_rule_impl.py`, `__init__.py`, `README.md` | `src/pypto_gym/ops/pypto_tensor/qwen3_6_27b/` |
| `qwen3_6_27b_pto_kernels/gated_delta_rule/{gated_delta_rule_golden.py, test_*.py, test_cases.json}` | `tests/ops/qwen3_6_27b/` |
...
> 然后 `ask_Qwen3.6-27B.py [--use_pypto]` 双模式验证。config.json 由权重目录提供，不单独归档。
