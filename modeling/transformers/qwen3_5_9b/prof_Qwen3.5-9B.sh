...
# msprof kernel-level profiling for Qwen3.5-9B — baseline vs PyPTO backend.
# Collects per-op NPU timing so the fused GatedDeltaRule kernel can be compared
# against the upstream chunk path.
#
# Usage:
#   MODEL_PATH=/path/to/Qwen3.5-9B bash prof_Qwen3.5-9B.sh
...
MODEL_ID="Qwen3.5-9B"
MODEL_PATH="${MODEL_PATH:?set MODEL_PATH to the Qwen3.5-9B weights directory}"
